...
import { Environment, ERC20ABI, PermitCreationError, PermitVariant, MsgRefundReq, MsgRefundReqResponse, MsgCreateRequestForFundsResponse, Universe, EVMVaultABI, MsgDoubleCheckTx, DepositVEPacket, createGrpcWebImpl, QueryClientImpl, msgpackableAxios, ChaindataMap, OmniversalChainID, LiFiAggregator, BebopAggregator, OmniversalRFF, MsgCreateRequestForFunds, getVaultContractMap, autoSelectSourcesV2, CurrencyID as CurrencyID$1, liquidateInputHoldings, determineDestinationSwaps, destinationSwapWithExactIn, createCosmosWallet, createCosmosClient } from '@avail-project/ca-common';
import { toHex } from 'viem/utils';
import posthog from 'posthog-js';
import Decimal from 'decimal.js';
import { keyDerivation } from '@starkware-industries/starkware-crypto-utils';
import { utils, TronWeb } from 'tronweb';
import { isAddress, formatUnits as formatUnits$1, createPublicClient, fallback, http, getContract, bytesToHex, encodeFunctionData, pad, hexToBigInt, maxUint256, toBytes, toHex as toHex$1, bytesToNumber, getAbiItem, encodeAbiParameters, keccak256, UserRejectedRequestError, hashMessage, bytesToBigInt, concat, parseSignature, encodePacked, serializeTransaction, webSocket, ContractFunctionExecutionError, hexToBytes, createWalletClient, custom, parseUnits as parseUnits$1 } from 'viem';
import { privateKeyToAccount } from 'viem/accounts';
import { createSiweMessage } from 'viem/siwe';
import axios from 'axios';
import { retry, remove, orderBy, uniqBy } from 'es-toolkit';
import { unpack, pack } from 'msgpackr';
import { isDeliverTxSuccess, isDeliverTxFailure } from '@cosmjs/stargate';
...
function bigIntToDecimalString(value, decimals) {
    const raw = formatUnits$1(value, decimals);
    if (raw.includes('.')) {
        const [i, f] = raw.split('.');
...
class BackendSimulationClient {
    ...
    async simulateBundleV2(request) {
        ...
        const { data } = await axios.post(new URL('/api/gas-estimation/bundleV2', this.baseUrl).href, request);
...
const createPublicClientWithFallback = (chain) => {
    return createPublicClient({
        transport: fallback(chain.rpcUrls.default.http.concat(chain.rpcUrls.default.publicHttp ?? []).map((s) => http(s))),
    });
};
...
function convertAddressByUniverse(input, universe) {
    ...
    if (universe === Universe.ETHEREUM || universe === Universe.TRON) {
        ...
        if (bytes.length === 32) {
            return inputIsString ? toHex$1(bytes.subarray(12)) : bytes.subarray(12);
        }
...
const getSupportedChains$1 = (env = Environment.CORAL) => {
...
const createRequestEVMSignature = async (evmRFF, evmAddress, client) => {
    ...
    const signature = toBytes(await client
        .signMessage({
        account: evmAddress,
        message: { raw: hash },
    })
...
const createRequestTronSignature = async (evmRFF, client) => {
    ...
    const hash = toHex$1(keccak256(msg, 'bytes'));
    ...
    return {
        requestHash: utils.message.hashMessage(hash),
    ...
    async iterate(chainList) {
        ...
        return Promise.all(values.map(async (b) => {
            ...
            if (this.isDeposit(b.contractAddress, b.universe)) {
                ...
                logger$b.debug('estimatedGasForDeposit', {
                    chainID: chain.id,
        ...
        fetchPriceOracle: async () => {
            ...
            if (data.PriceOracleData?.priceData?.length) {
                const oracleRates = data.PriceOracleData?.priceData.map((data) => ({
                    ...
                    tokenAddress: convertAddressByUniverse(toHex$1(data.tokenAddress), data.universe),
        ...
        feeData.fee.collection =
            p.value.ProtocolFees?.collectionFees.map((fee) => {
                return {
                    ...
                    tokenAddress: convertAddressByUniverse(toHex$1(fee.tokenAddress), fee.universe),
        ...
        feeData.solverRoutes =
            s.value.solverData[0]?.advertisedFees.map((s) => {
                return {
                    ...
                    destinationTokenAddress: convertAddressByUniverse(toHex$1(s.destinationTokenAddress), s.destinationUniverse),
                    destinationUniverse: s.destinationUniverse,
                    feeBP: s.feeBP,
                    sourceChainID: bytesToNumber(s.sourceChainID),
                    sourceTokenAddress: convertAddressByUniverse(toHex$1(s.sourceTokenAddress), s.sourceUniverse),
        ...
        vscSBCTx: async (input) => {
            ...
            try {
                ...
                for await (const response of connection.source) {
                    ...
                    ops.push([bytesToBigInt(input[data.part_idx].chain_id), toHex$1(data.tx_hash)]);
                    count += 1;
...
const createBatchedCallSignature = (batchedCalls, nonce, chain, address, account, deadline) => {
    return account.signTypedData({
        ...
        message: {
            ...
            deadline,
            executor: toHex$1(ZERO_BYTES_20),
            keyHash: toHex$1(ZERO_BYTES_32),
            nonce,
        },
...
const createSBCTxFromCalls = async ({ cache, calls, chainID, ephemeralAddress, ephemeralWallet, publicClient, }) => {
    const nonce = bytesToBigInt(await cryptoGetRandomValues(new Uint8Array(24))) << 64n;
...
const convertToEVMAddress = (addr) => {
    ...
    if (address.length === 20) {
        return toHex$1(address);
    }
    if (address.length === 32) {
        return toHex$1(address.subarray(12));
    }
    throw Errors.invalidAddressLength('evm');
};
...
const EXPECTED_CALIBUR_CODE = concat(['0xef0100', CALIBUR_ADDRESS]);
...
const createPermitApprovalTx = async ({ contractAddress, owner, ownerWallet, spender, variant, version, amount, }) => {
    ...
    return {
        data: variant === PermitVariant.PolygonEMT
            ? encodeFunctionData({
                abi: ERC20PermitABI,
...
const getAnkrBalances = async (walletAddress, chainList, removeTransferFee) => {
    const publicClients = {};
    const res = await axios.post('https://rpcs.avail.so/multichain', {
        ...
        promises.push((async () => {
            ...
            if (asset.tokenType === 'NATIVE') {
                ...
                balance = new Decimal(asset.balance).gt(transferFee)
                    ? Decimal.sub(asset.balance, transferFee).toFixed(asset.tokenDecimals, Decimal.ROUND_FLOOR)
                    : '0';
                logger$8.debug('getAnkrBalances', {
                    chainID,
                    newBalance: balance,
...
const toFlatBalance = (assets, convertAddressToBytes32 = true, currentChainID, selectedTokenAddress) => {
    ...
    return assets
        ...
        .filter((b) => b.universe === Universe.ETHEREUM &&
...
const vscBalancesToAssets = (chainList, evmBalances = [], tronBalances = []) => {
    ...
    for (const balance of vscBalances) {
        for (const currency of balance.currencies) {
            ...
            const tokenAddress = convertAddressByUniverse(toHex$1(currency.token_address), balance.universe);
...
const createSweeperTxs = ({ cache, chainID, COTCurrencyID, receiver, sender, tokenAddress, }) => {
    const txs = [];
    if (!tokenAddress) {
        const currency = ChaindataMap.get(new OmniversalChainID(Universe.ETHEREUM, chainID)).Currencies.find((c) => c.currencyID === COTCurrencyID);
        if (!currency) {
...
const getBalanceSlot = ({ tokenSymbol, chainId, userAddress, }) => {
    ...
    const userBalanceSlot = keccak256(encodePacked(['bytes32', 'uint256'], [pad(userAddress, { size: 32 }), BigInt(balanceSlot)]));
...
const generateStateOverride = (params) => {
    const amountInHex = toHex$1(params.amount * 2n);
...
const createRFFromIntent = async (intent, options, destinationUniverse) => {
    ...
    for (const universe of universes) {
        ...
        if (universe === Universe.TRON) {
            ...
            parties.push({
                address: convertTo32BytesHex(tronHexToEvmAddress(TronWeb.address.toHex(options.tron.address))),
                universe,
            });
        }
    }
    ...
    for (const universe of universes) {
        ...
        if (universe === Universe.TRON) {
            ...
            signatureData.push({
                address: convertTo32Bytes$1(tronHexToEvmAddress(TronWeb.address.toHex(options.tron.address))),
                requestHash,
                signature,
                universe,
            });
...
class BridgeAndExecuteQuery {
    ...
    async estimateBridgeAndExecute(params) {
        ...
        const [gasUsed, gasPriceRecommendations, balances, l1Fee] = await Promise.all([
            ...
            getL1Fee(execute.to, dstChain, serializeTransaction({
                chainId: dstChain.id,
...
let BridgeHandler$1 = class BridgeHandler {
    ...
    async waitForFill(requestHash, intentID, waitForDoubleCheckTx) {
        ...
        if (this.params.dstChain.universe === Universe.ETHEREUM) {
            promisesToRace.push(waitForIntentFulfilment(createPublicClient({
        ...
        for (const [i, s] of sources.entries()) {
            ...
            if (s.universe === Universe.ETHEREUM && isNativeAddress(s.universe, s.tokenAddress)) {
                ...
                const { request } = await publicClient.simulateContract({
                    ...
                    args: [omniversalRFF.asEVMRFF(), toHex$1(evmSignatureData.signature), BigInt(i)],
                    chain: chain,
                ...
                const txWrap = await provider.transactionBuilder.triggerSmartContract(TronWeb.address.fromHex(vaultContractAddress), '', {
                    txLocal: true,
                    input: encodeFunctionData({
                        abi: EVMVaultABI,
                        functionName: 'deposit',
                        args: [omniversalRFF.asEVMRFF(), toHex$1(tronSignatureData.signature), BigInt(i)],
                    }),
        ...
        try {
            for (const source of input) {
                ...
                if (currency.permitVariant === PermitVariant.Unsupported || chain.id === 1) {
                    if (chain.universe === Universe.ETHEREUM) {
                        const h = await this.options.evm.client
                            ...
                            if (e instanceof ContractFunctionExecutionError) {
                                const isUserRejectedRequestError = e.walk((e) => e instanceof UserRejectedRequestError) instanceof
                        ...
                        const tx = await provider.transactionBuilder.triggerSmartContract(TronWeb.address.fromHex(source.tokenContract), 'approve(address,uint256)', {
                            txLocal: true,
                        }, [
                    ...
                    const signed = parseSignature(await signPermitForAddressAndValue(currency, this.options.evm.client, publicClient, account, vc, source.amount).catch((e) => {
                        if (e instanceof ContractFunctionExecutionError) {
                            const isUserRejectedRequestError = e.walk((e) => e instanceof UserRejectedRequestError) instanceof
                                UserRejectedRequestError;
                            if (isUserRejectedRequestError) {
...
const _exactOutRoute = async (input, params) => {
    ...
    const cotSymbol = CurrencyID$1[params.cotCurrencyID];
    ...
    const fetchDestinationSwapDetails = async () => {
        ...
        if (!equalFold(input.toTokenAddress, dstChainCOTAddress)) {
            const [ds, dgs] = await Promise.all([
                determineDestinationSwaps(userAddressInBytes, dstOmniversalChainID, {
                    amount: BigInt(input.toAmount),
                    tokenAddress: convertTo32Bytes(input.toTokenAddress),
                }, params.aggregators),
                gasInCOT.isZero()
                    ? null
                    : destinationSwapWithExactIn(userAddressInBytes, dstOmniversalChainID, mulDecimals(gasInCOT, dstChainCOT.decimals), EADDRESS_32_BYTES, params.aggregators),
            ]);
            destinationSwap = ds;
            if (dgs) {
                gasSwap = {
                    ...
                    originalHolding: {
                        ...
                        value: 0,
                        decimals: dstChainCOT.decimals,
                        symbol: CurrencyID$1[dstChainCOT.currencyID],
                    },
                };
            }
        }
        ...
        const max = applyBuffer(min, BUFFER_EXACT_OUT.DESTINATION_SWAP).toDP(dstChainCOT.decimals, Decimal.ROUND_CEIL);
        return {
    ...
    const estimatedCollectionFee = estimateCollectionFee(uniqBy(balances, (b) => b.chainID).map((b) => {
    ...
    const { quotes: sourceSwapQuotes, usedCOTs } = await autoSelectSourcesV2(userAddressInBytes, sortedBalances.map((balance) => ({
    ...
    let dstTotalCOTAmount = new Decimal(dstEOAToEphTx ? divDecimals(dstEOAToEphTx.amount, dstChainCOT.decimals) : 0);
    for (const cot of usedCOTs) {
        assetsUsed.push({
            ...
            symbol: CurrencyID$1[cot.cur.currencyID],
        });
    ...
    for (const swap of sourceSwaps) {
        ...
        if (bAsset) {
            bAsset.ephemeralBalance = Decimal.add(bAsset.ephemeralBalance, divDecimals(outputAmount, token.decimals));
        }
        else {
            bridgeAssets.push({
                chainID: Number(swap.req.chain.chainID),
...
const _exactInRoute = async (input, params) => {
    ...
    for (const source of srcBalances) {
        const cot = ChaindataMap.get(new OmniversalChainID(Universe.ETHEREUM, source.chainID))?.Currencies.find((c) => c.currencyID === params.cotCurrencyID);
        if (cot &&
    ...
    if (isSrcSwapRequired) {
        const response = await liquidateInputHoldings(userAddressInBytes, srcBalances.map((b) => ({
    ...
    const fetchDestinationSwapDetails = async () => {
        ...
        dstSwapInputAmountInDecimal = dstSwapInputAmountInDecimal.toDP(dstChainCOT.decimals, Decimal.ROUND_FLOOR);
        // If toTokenAddress is not same as cot then create dstSwap
        if (!equalFold(input.toTokenAddress, dstChainCOTAddress)) {
            destinationSwap = await destinationSwapWithExactIn(userAddressInBytes, dstOmniversalChainID, mulDecimals(dstSwapInputAmountInDecimal, dstChainCOT.decimals), convertTo32Bytes(input.toTokenAddress), params.aggregators, dstChainCOT.currencyID);
...
const createIntent = ({ assets, feeStore, output, address, }) => {
    ...
    const sortedAssets = [...assets].sort((a, b) => {
        ...
        return Decimal.sub(Decimal.add(a.eoaBalance, a.ephemeralBalance), Decimal.add(b.eoaBalance, b.ephemeralBalance)).toNumber(); //sort others by balance
    });
    for (const asset of sortedAssets) {
        ...
        const estimatedBorrowFromThisChain = Decimal.add(asset.eoaBalance.toString(), asset.ephemeralBalance.toString()).lte(unaccountedBalance)
        ...
        if (Decimal.add(asset.eoaBalance, asset.ephemeralBalance).lte(unaccountedBalance2)) {
            logger$3.debug('createBridgeRFF:2.1', {
                assetBalance: Decimal.add(asset.eoaBalance.toString(), asset.ephemeralBalance.toString()).toFixed(),
                unaccountedBalance: unaccountedBalance2.toFixed(),
            });
            borrowFromThisChain = Decimal.add(asset.eoaBalance.toString(), asset.ephemeralBalance.toString());
...
const createBridgeRFF = async ({ config, input, output, }) => {
    ...
    const createRFF = async () => {
        ...
        for (const s of omniversalRFF.protobufRFF.sources) {
            doubleCheckTxMap[bytesToNumber(s.chainID)] = createDoubleCheckTx(s.chainID, config.cosmos, intentID);
        }
        return {
    ...
    for (const [index, source] of sources.entries()) {
        ...
        console.log({
            argsForRFFDeposit: [
                ...
                toHex$1(evmSignatureData.signature),
                BigInt(index),
            ],
        });
        tx.push({
            data: encodeFunctionData({
                abi: EVMVaultABI,
                args: [omniversalRFF.asEVMRFF(), toHex$1(evmSignatureData.signature), BigInt(index)],
                functionName: 'deposit',
            }),
...
class SourceSwapsHandler {
    ...
    *iterate(input) {
        ...
        const quoteResponses = await retry(() => {
            ...
            for (const fChain of failedChains) {
                ...
                for (const oq of oldQuotes) {
                    ...
                    quoteRequests.push(liquidateInputHoldings(oq.req.userAddress, [
                        {
                            ...oq.originalHolding,
                            amount: oq.quote.inputAmount,
                            tokenAddress: oq.req.inputToken,
    ...
    groupAndOrder(input) {
        return Map.groupBy(orderBy(input, [
            (s) => 
...
const swap = async (input, options, COT = CurrencyID$1.USDC) => {
    ...
    // Swap Intent hook handling
    {
        const destinationTokenDetails = {
            amount: divDecimals(input.mode === SwapMode.EXACT_OUT
                ? input.data.toAmount
    ...
    const dstSwapHandler = new DestinationSwapHandler(destination, dstTokenInfo, {
        ...
        amount: input.mode === SwapMode.EXACT_OUT
            ? input.data.toAmount
...
class CA {
    ...
    constructor(config = {
        ...
        const defaultChain = this._networkConfig.NETWORK_HINT === Environment.FOLLY
            ? SUPPORTED_CHAINS.SEPOLIA
            : SUPPORTED_CHAINS.ETHEREUM;
        this._siweChain = config.siweChain ?? defaultChain;
        this._siweParams = config.siweParams ?? {
            scheme: locationProtocol(),
            domain: locationHost(),
    ...
    _setTronAdapter = async (adapter) => {
        ...
        this._tron = {
            adapter,
            address: tronHexToEvmAddress(utils.address.toHex(adapter.address)),
        };
    };
...
const parseUnits = parseUnits$1;
const formatUnits = formatUnits$1;
...
export { AnalyticsManager, BRIDGE_STEPS, CHAIN_METADATA, CaliburSBCTypes, DESTINATION_SWAP_TOKENS, ERROR_CODES, LOG_LEVEL, LOG_LEVEL_NAME, MAINNET_CHAINS$1 as MAINNET_CHAINS, MAINNET_CHAIN_IDS, NEXUS_EVENTS, NexusAnalyticsEvents, NexusError, NexusSDK, SUPPORTED_CHAINS, SWAP_STEPS, SwapMode, TESTNET_CHAINS$1 as TESTNET_CHAINS, TESTNET_CHAIN_IDS, TESTNET_TOKEN_METADATA, TOKEN_CONTRACT_ADDRESSES, TOKEN_METADATA, formatTokenBalance, formatTokenBalanceParts, formatUnits, getCoinbaseRates, getLogger, getSupportedChains, isSupportedToken, isValidAddress, logger$e as logger, parseUnits, setExceptionReporter, setLogLevel, truncateAddress };
