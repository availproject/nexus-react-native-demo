...
export const Chaindata = RawData.map((ch) => {
    const ch32 = Buffer.from(hexToBytes(ch.ChainID32));
    const currencies = ch.Currencies.map((cur) => {
        return new Currency(cur.CurrencyID, hexToBytes(cur.TokenContractAddress), cur.TokenDecimals, cur.PermitVariant, cur.PermitContractVersion, cur.IsGasToken);
    });
    return {
...
export const ChaindataMap = new ChainIDKeyedMap(Chaindata.map((ch) => [ch.ChainID, ch]));
