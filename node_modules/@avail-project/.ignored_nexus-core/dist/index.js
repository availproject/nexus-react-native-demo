...
var caCommon = require('@avail-project/ca-common');
...
if (proto && typeof proto.writeUint32BE !== 'function') {
    if (typeof proto.writeUInt32BE === 'function') {
        ...
        proto.writeUint32BE = function (value, offset = 0) {
            offset = offset >>> 0;
        ...
        proto.writeUint32LE = function (value, offset = 0) {
            offset = offset >>> 0;
            const normalized = Number(value) >>> 0;
...
const ALLOWANCE_APPROVAL_REQUEST = (chain) => ({
...
const ALLOWANCE_APPROVAL_MINED = (chain) => ({
...
const INTENT_DEPOSIT_REQUEST = (id, amount, chain) => ({
    type: 'INTENT_DEPOSIT',
    typeID: `ID_${id}`,
    data: {
...
const INTENT_COLLECTION = (id, total, txHash, explorerUrl) => ({
...
class LogsAPI {
    ...
    getLoggerProvider() {
        var _a, _b;
        return ((_b = (_a = _global$1[GLOBAL_LOGS_API_KEY]) === null || _a === void 0 ? void 0 : _a.call(_global$1, API_BACKWARDS_COMPATIBILITY_VERSION)) !== null && _b !== void 0 ? _b : this._proxyLoggerProvider);
    }
    /**
...
function getOtlpEncoder(options) {
    ...
    return {
        ...
        encodeSpanContext: identity$1 ,
        encodeOptionalSpanContext: identity$1 ,
    };
}
...
class LoggerProvider {
    ...
    getLogger(name, version, options) {
        ...
        if (!this._sharedState.loggers.has(key)) {
            this._sharedState.loggers.set(key, new Logger$1({ name: loggerName, version, schemaUrl: options?.schemaUrl }, this._sharedState));
        }
...
function detectPlatform() {
    ...
    const isReactNative = hasNavigator &&
        (navigator.product === 'ReactNative' || navigator.userAgent?.includes('React Native'));
    const hasHermes = typeof global !== 'undefined' && !!global.HermesInternal;
    const hasExpo = typeof global !== 'undefined' &&
        (global.expo != null ||
            global.__expo != null ||
            global.ExpoModules != null);
...
async function cryptoGetRandomValues(bytes) {
...
async function getOrGenerateClientId() {
    ...
    if (!clientId) {
        ...
        clientId = utils.toHex(await cryptoGetRandomValues(bytes));
        storageSetItem(KEY, clientId);
    }
    return clientId;
}
function getNetworkName(networkConfig) {
    return caCommon.Environment[networkConfig.NETWORK_HINT];
}
const setLoggerProvider = async (networkConfig) => {
    if (!telemetryLogger) {
...
const Errors = {
    ...
    sdkInitStateNotExpected: (state) => createError(ERROR_CODES.SDK_INIT_STATE_NOT_EXPECTED, 'Unexpected init SDK state', {
        details: { state },
    }),
    accountConnectionFailed: () => createError(ERROR_CODES.CONNECT_ACCOUNT_FAILED, 'Account failed to connect from connector'),
    environmentNotSupported: (environment) => createError(ERROR_CODES.ENVIRONMENT_NOT_SUPPORTED, 'Environment not supported yet', {
        details: { environment },
    }),
    environmentNotKnown: () => createError(ERROR_CODES.ENVIRONMENT_NOT_KNOWN, 'Environment not known/mapped'),
    invalidAllowance: (expected, got) => createError(ERROR_CODES.INVALID_VALUES_ALLOWANCE_HOOK, 'Invalid allowance values passed. The length of allowances should equal input lengths.', {
        context: 'onAllowance:allow()',
        details: { expectedLength: expected, receivedLength: got },
    }),
    chainNotFound: (chainId) => createError(ERROR_CODES.CHAIN_NOT_FOUND, `Chain not found: ${chainId}`, {
        details: { chainId },
    }),
    chainDataNotFound: (chainId) => createError(ERROR_CODES.CHAIN_DATA_NOT_FOUND, `Chain data not found for chain: ${chainId}`, {
        details: { chainId },
    }),
    assetNotFound: (tokenSymbol) => createError(ERROR_CODES.ASSET_NOT_FOUND, `Asset not found in UserAssets: ${tokenSymbol}`, {
        details: { tokenSymbol },
    }),
    internal: (msg, details) => createError(ERROR_CODES.INTERNAL_ERROR, `Internal error: ${msg}`, {
        details,
    }),
    tokenNotSupported: (address, chainId, additionalMessage) => createError(ERROR_CODES.TOKEN_NOT_SUPPORTED, `Token/Asset with address ${address} is not supported on chain ${chainId}.\n${additionalMessage}`, {
        details: { address, chainId },
    }),
    universeNotSupported: () => createError(ERROR_CODES.UNIVERSE_NOT_SUPPORTED, 'Universe not supported'),
    tokenNotFound: (symbol, chainId) => createError(ERROR_CODES.TOKEN_NOT_SUPPORTED, `Token with symbol ${symbol} not found on chain ${chainId}`, {
        details: { symbol, chainId },
    }),
    tronDepositFailed: (result) => createError(ERROR_CODES.TRON_DEPOSIT_FAIL, 'Tron deposit transaction failed.', {
        details: { result },
    }),
    tronApprovalFailed: (result) => createError(ERROR_CODES.TRON_APPROVAL_FAIL, 'Tron approval transaction failed.', {
        details: { result },
    }),
    liquidityTimeout: () => createError(ERROR_CODES.LIQUIDITY_TIMEOUT, 'Timed out waiting for fulfilment.'),
    userDeniedIntent: () => createError(ERROR_CODES.USER_DENIED_INTENT, 'User rejected the intent.'),
    userRejectedAllowance: () => createError(ERROR_CODES.USER_DENIED_ALLOWANCE, 'User rejected the allowance.'),
    userRejectedIntentSignature: () => createError(ERROR_CODES.USER_DENIED_INTENT_SIGNATURE, 'User rejected signing the intent hash.'),
    insufficientBalance: (msg) => createError(ERROR_CODES.INSUFFICIENT_BALANCE, `Insufficient balance to proceed. ${msg}`),
    walletNotConnected: (walletType) => createError(ERROR_CODES.WALLET_NOT_CONNECTED, `Wallet is not connected for ${walletType}`),
    userRejectedSIWESignature: () => createError(ERROR_CODES.USER_DENIED_SIWE_SIGNATURE, 'User rejected SIWE signature.'),
    vscError: (msg, data) => createError(ERROR_CODES.INTERNAL_ERROR, `VSC: ${msg}`, {
        details: {
            data,
        },
    }),
    cosmosError: (msg) => createError(ERROR_CODES.COSMOS_ERROR, `COSMOS: ${msg}`),
    gasPriceError: (result) => createError(ERROR_CODES.FETCH_GAS_PRICE_FAILED, 'rpc: estimateMaxFeePerGas failed', {
        details: {
            result,
        },
    }),
    unknownSignatureType: () => createError(ERROR_CODES.UNKNOWN_SIGNATURE, 'Unknown signature type'),
    quoteFailed: (message) => createError(ERROR_CODES.QUOTE_FAILED, `Quote failed: ${message}`),
    swapFailed: (message) => createError(ERROR_CODES.SWAP_FAILED, `Swap failed: ${message}`),
    ratesChangedBeyondTolerance: (rate, tolerance) => createError(ERROR_CODES.RATES_CHANGED_BEYOND_TOLERANCE, `Rates changed beyond tolerance. Rate: ${rate}\nTolerance:${tolerance}`),
    slippageError: (msg) => createError(ERROR_CODES.SLIPPAGE_EXCEEDED_ALLOWANCE, `rpc: slippage exceeded - ${msg}`),
    vaultContractNotFound: (chainId) => createError(ERROR_CODES.VAULT_CONTRACT_NOT_FOUND, `vault contract not found for chain ${chainId.toString()}`),
    simulationError: (msg) => createError(ERROR_CODES.SIMULATION_FAILED, `tenderly simulation failed: ${msg}`),
    rFFFeeExpired: () => createError(ERROR_CODES.RFF_FEE_EXPIRED, 'fee is not adequate'),
    destinationRequestHashNotFound: () => createError(ERROR_CODES.DESTINATION_REQUEST_HASH_NOT_FOUND, 'requestHash not found for destination'),
    transactionTimeout: (timeout) => createError(ERROR_CODES.TRANSACTION_TIMEOUT, `⏰ Timeout: Transaction not confirmed within ${timeout}s`),
    transactionReverted: (txHash) => createError(ERROR_CODES.TRANSACTION_REVERTED, `Transaction reverted: ${txHash}`),
    invalidInput: (msg) => createError(ERROR_CODES.INVALID_INPUT, `input invalid: ${msg}`),
    invalidAddressLength: (addressType, additionalMessage) => createError(ERROR_CODES.INVALID_ADDRESS_LENGTH, `Invalid ${addressType} address length: ${additionalMessage}`, { details: { type: addressType } }),
    noBalanceForAddress: (address) => {
...
const CREATE_PERMIT_EOA_TO_EPHEMERAL = (completed, symbol, chain) => ({
    chain: {
        id: chain.id,
        name: chain.name,
    },
...
const SOURCE_SWAP_BATCH_TX = (completed) => ({
    completed,
...
function bigIntToDecimalString(value, decimals) {
    const raw = viem.formatUnits(value, decimals);
    if (raw.includes('.')) {
        const [i, f] = raw.split('.');
...
function normalizeValue(value, decimals) {
    let decimalStr;
...
function countLeadingZeros(fracRaw) {
    let count = 0;
...
function formatTiny(negative, fracRaw, zeros, opts) {
 ...
 */
function formatTokenBalanceParts$1(value, { decimals, symbol, significantDigits = 4, maxFractionDigits = 4, tinyThresholdPower = -4, zeroCompress = true, thousandSeparator = false, trimTrailingZeros = true, approxTilde = true, } = {}) {
 ...
 *   use `formatTokenBalanceParts` which returns structured parts.
 */
function formatTokenBalance$1(value, options) {
    return formatTokenBalanceParts$1(value, options).text;
}
...
class Logger {
    ...
    internalLog(level, message, params) {
        ...
        if (level === LOG_LEVEL.ERROR || level === LOG_LEVEL.WARNING) {
            const cause = params && typeof params === 'object' && 'cause' in params
                ? params.cause
                : 'unknown|not_mapped';
            try {
                telemetryLogger?.emit({
...
class PostHogProvider {
    ...
    initialize() {
        ...
        try {
            posthog__default.default.init(this.config.apiKey, {
    ...
    track(event, properties) {
        ...
        try {
            posthog__default.default.capture(event, properties);
        }
        catch (error) {
    ...
    identify(userId, properties) {
        ...
        try {
            posthog__default.default.identify(userId, {
                ...properties,
    ...
    register(properties) {
        ...
        try {
            posthog__default.default.register(properties);
        }
        catch (error) {
     ...
     */
    getPostHog() {
        return posthog__default.default;
    }
}
...
class SessionManager {
     ...
     */
    getSessionStartProperties(url, referrer) {
        return {
     ...
     */
    getSessionEndProperties() {
        const sessionDuration = Date.now() - this.sessionStartTime;
        const successRate = this.operationsAttempted > 0
...
function getWalletType(provider) {
    if (!provider)
        return 'Unknown';
    ...
    if (provider.isAmbire)
        return 'Ambire Wallet';
    if (provider.isMetaMask)
        return 'MetaMask'; // placing metamask last to avoid false positives
 ...
 */
function extractSwapProperties(swaps) {
    if (!swaps)
        return {};
    const props = {};
    ...
    if (swaps.sourceSwaps) {
        props.sourceSwaps = swaps.sourceSwaps.map((s) => ({
            ...
            sources: s.swaps.map((swp) => ({
                ...
                amount: new Decimal__default.default(swp.inputAmount.toString())
                    .div(Decimal__default.default.pow(10, swp.inputDecimals))
                    .toDecimalPlaces(swp.inputDecimals)
                    .toFixed(),
            })),
            destinations: s.swaps.map((swp) => ({
                tokenContract: swp.outputContract,
                amount: new Decimal__default.default(swp.outputAmount.toString())
                    .div(Decimal__default.default.pow(10, swp.outputDecimals))
    ...
    if (swaps.swapRoute?.bridge) {
        props.bridge = {
            ...
            destination: {
                ...
                amount: new Decimal__default.default(swaps.swapRoute.bridge.amount).toFixed(),
            },
        };
    }
    ...
    if (swaps.destinationSwap) {
        props.destinationSwap = {
            ...
            source: swaps.destinationSwap.swaps.map((s) => ({
                ...
                amount: new Decimal__default.default(s.inputAmount.toString())
                    .div(Decimal__default.default.pow(10, s.inputDecimals))
                    .toDecimalPlaces(s.inputDecimals)
                    .toFixed(),
            })),
            destination: swaps.destinationSwap.swaps.map((s) => ({
                tokenContract: s.outputContract,
                amount: new Decimal__default.default(s.outputAmount.toString())
                    .div(Decimal__default.default.pow(10, s.outputDecimals))
 ...
 */
function extractBridgeProperties(intent) {
    if (!intent)
        return {};
    return {
        bridge: {
            sources: intent.sources?.map((s) => ({
                chainId: s.chainID,
                token: s.contractAddress,
                amount: new Decimal__default.default(s.amount).toFixed(),
            })),
            destination: {
                chainId: intent.destination?.chainID,
                // token: intent.token.symbol,
                amount: new Decimal__default.default(intent.destination?.amount || 0).toFixed(),
            },
            fees: intent.fees,
        },
    };
...
class AnalyticsManager {
    ...
    constructor(network, config) {
        ...
        try {
            ...
            logger$e.debug('analytics init failed', e);
    ...
    trackSessionStart() {
        ...
        this.track(NexusAnalyticsEvents.SESSION_STARTED, this.session.getSessionStartProperties(url, referrer));
        if (this.config.debug) {
     ...
     */
    async hashWalletAddress(address) {
        try {
    ...
    endOperation(operationId, result) {
        ...
        if (perfProperties) {
            ...
            this.track(NexusAnalyticsEvents.OPERATION_PERFORMANCE, perfProperties);
        }
    }
    /**
    ...
    async trackOperation(operationName, fn, metadata) {
        ...
        try {
            ...
            this.endOperation(operationId, { success: false, error: error });
            throw error;
        }
    }
    /**
    ...
    trackSessionEnd() {
        if (!this.enabled)
            return;
        this.track(NexusAnalyticsEvents.SESSION_ENDED, this.session.getSessionEndProperties());
        if (this.config.debug) {
 ...
 */
function createBackendSimulationClient(config) {
...
class BackendSimulationClient {
    ...
    async simulateBundleV2(request) {
        logger$e.debug('DEBUG simulateBundle - request:', JSON.stringify(request, null, 2));
        const { data } = await axios__default.default.post(new URL('/api/gas-estimation/bundleV2', this.baseUrl).href, request);
...
const getAllowance = async (chain, address, tokenContract, chainList) => {
    logger$d.debug('getAllowance', {
        tokenContract,
        ZERO_ADDRESS,
        chain,
        address,
...
const erc20GetAllowance = (params, client) => {
...
const getAllowances = async (input, chainList) => {
    const values = {};
    const promises = [];
...
const waitForIntentFulfilment = async (publicClient, vaultContractAddr, requestHash, ac) => {
    return new Promise((resolve) => {
        const unwatch = publicClient.watchContractEvent({
            ...
            onLogs: (logs) => {
                logger$d.debug('waitForIntentFulfilment', { logs });
                ac.abort();
        ...
        ac.signal.addEventListener('abort', () => {
            logger$d.debug('waitForIntentFulfilment: got abort, going to unwatch');
            unwatch();
...
const getL1Fee = async (toAddress, chain, input = '0x') => {
    let fee = 0n;
...
const fetchL1Fee = async (toAddress, chain, input) => {
...
const waitForTxReceipt = async (hash, publicClient, confirmations = 1, timeout = 60000) => {
...
const switchChain = async (client, chain) => {
    ...
    try {
        ...
        logger$d.error('switchChain failed, trying addChain', outerErr);
        try {
            await client.addChain({ chain });
            await client.switchChain({ id: chain.id });
        }
        catch (inner) {
            logger$d.error('Unable to add/switch chain', inner);
            throw inner;
        }
    }
    const after = await client.getChainId();
    if (after !== chain.id) {
        logger$d.error('Wallet did not switch chains even though no error was thrown');
...
async function signPermitForAddressAndValue(cur, client, publicClient, account, spender, value, ddl) {
...
const createPublicClientWithFallback = (chain) => {
    return viem.createPublicClient({
        transport: viem.fallback(chain.rpcUrls.default.http.concat(chain.rpcUrls.default.publicHttp ?? []).map((s) => viem.http(s))),
    });
};
const getPctGasBufferByChain = (chainId) => {
...
const cosmosFeeGrant = async (address, cosmosClient, vscClient) => {
    try {
        await cosmosClient.getAccount(address);
    }
    catch (e) {
        logger$c.error('Requesting a fee grant', e, { cause: 'FEE_GRANT_REQUESTED' });
        const response = await vscClient.vscCreateFeeGrant(address);
        logger$c.debug('Fee grant response', response);
        return;
    }
};
const cosmosFillCheck = async (intentID, cosmosClient, ac) => {
    return Promise.any([
        cosmosClient.waitForCosmosFillEvent(intentID, ac),
        cosmosClient.checkIntentFilled(intentID),
    ]);
};
const cosmosCreateRFF = async ({ address, client, msg, }) => {
    try {
...
const cosmosRefundIntent = async ({ address, client, intentID, }) => {
    try {
        ...
        try {
            ...
            logger$c.error('Refund failed', e, { cause: 'REFUND_FAILED' });
            throw e;
        }
    }
    finally {
...
const cosmosCreateDoubleCheckTx = async ({ address, client, msg, }) => {
    try {
        logger$c.debug('cosmosCreateDoubleCheckTx', { doubleCheckMsg: msg });
        const res = await client.signAndBroadcast(address, [
            {
        ...
        logger$c.debug('double check response', { doubleCheckTx: res });
    }
    finally {
        client.disconnect();
    }
...
function convertAddressByUniverse(input, universe) {
    ...
    if (universe === caCommon.Universe.ETHEREUM || universe === caCommon.Universe.TRON) {
        if (bytes.length === 20) {
...
const storeIntentHashToStore = (address, id, createdAt = Date.now()) => {
    let intents = [];
    const fetchedIntents = storageGetItem(getIntentKey(address));
    if (fetchedIntents) {
        intents = JSON.parse(fetchedIntents) ?? [];
    }
    intents.push({ createdAt, id });
    storageSetItem(getIntentKey(address), JSON.stringify(intents));
};
...
const getExpiredIntents = (address) => {
    ...
    logger$b.debug('getExpiredIntents', { intents });
    ...
    storageSetItem(getIntentKey(address), JSON.stringify(nonExpiredIntents));
    return expiredIntents;
};
const refundExpiredIntents = async ({ address, evmAddress, client, analytics, }) => {
    logger$b.debug('Starting check for expired intents at ', new Date());
    const expIntents = getExpiredIntents(evmAddress);
    const failedRefunds = [];
    for (const intent of expIntents) {
        logger$b.debug(`Starting refund for: ${intent.id}`);
        ...
        try {
            ...
            logger$b.debug('Refund failed', e);
...
const getSupportedChains$1 = (env = caCommon.Environment.CORAL) => {
...
const createRequestEVMSignature = async (evmRFF, evmAddress, client) => {
    logger$b.debug('createReqEVMSignature', { evmRFF });
    ...
    const signature = viem.toBytes(await client
        .signMessage({
        account: evmAddress,
        message: { raw: hash },
    })
        .catch((e) => {
        if (e instanceof viem.UserRejectedRequestError) {
...
const createRequestTronSignature = async (evmRFF, client) => {
    logger$b.debug('createReqEVMSignature', { evmRFF });
    const abi = viem.getAbiItem({ abi: caCommon.EVMVaultABI, name: 'deposit' });
    const msg = viem.encodeAbiParameters(abi.inputs[0].components, [
        evmRFF.sources,
    ...
    const hash = viem.toHex(viem.keccak256(msg, 'bytes'));
    ...
    return {
        requestHash: tronweb.utils.message.hashMessage(hash),
        signature: viem.toBytes(sig),
    };
};
...
const convertGasToToken = (token, oraclePrices, destinationChainID, destinationUniverse, gas) => {
    ...
    const gasTokenInUSD = oraclePrices
        .find((rate) => rate.chainId === destinationChainID && equalFold(rate.tokenAddress, ZERO_ADDRESS))
        ?.priceUsd.toFixed() ?? '0';
...
const evmWaitForFill = async (vaultContractAddress, publicClient, requestHash, intentID, cosmosQueryClient) => {
...
const createDepositDoubleCheckTx = (chainID, cosmos, intentID) => {
    ...
    getBridgeAssets(dstChainId) {
        return this.value.breakdown
            ...
            return {
                ...
                decimals: b.decimals,
                balance: new Decimal__default.default(b.balance),
            };
        });
    }
    ...
    async iterate(chainList) {
        ...
        for (const b of values) {
            ...
            if (this.isDeposit(b.contractAddress, b.universe)) {
                const ESTIMATED_DEPOSIT_GAS = 200000n;
                ...
                const estimatedGasForDeposit = divDecimals(ESTIMATED_DEPOSIT_GAS * gasUnitPrice, chain.nativeCurrency.decimals);
...
async function waitForTronDepositTxConfirmation(hash, vaultContractAddress, tronWeb, owner, options = {}) {
    const { timeout = 120000, interval = 3000 } = options;
    const startTime = Date.now();
    logger$b.debug('Waiting for confirmation of tron tx');
    ...
    while (Date.now() - startTime < timeout) {
        try {
            const result = await tronWeb.transactionBuilder.triggerConstantContract(tronWeb.utils.address.fromHex(vaultContractAddress), '', {
                input,
            }, [], tronWeb.utils.address.fromHex(owner));
            logger$b.debug('requestHashWitnessedOnTron', {
                result,
            });
            if (result.Error) {
                throw Errors.internal(result.Error);
            }
            const requestState = viem.bytesToNumber(result.constant_result[0]);
            if (requestState === 0) {
            ...
            logger$b.error('Error while checking transaction:', err, {
                cause: 'TRANSACTION_CHECK_ERROR',
            });
            // Don’t throw yet; continue polling
        }
        logger$b.debug('⏳ Still waiting...');
        await new Promise((resolve) => setTimeout(resolve, interval));
    }
    throw Errors.transactionTimeout(timeout / 1000);
}
function pctAdditionWithSuggestion(base, percentage) {
    const pctAmount = BigInt(new Decimal__default.default(base).mul(percentage).toFixed(0, Decimal__default.default.ROUND_CEIL));
    const value = base + pctAmount;
    const reducedValue = base + (pctAmount * 70n) / 100n;
    return [reducedValue, value];
}
async function waitForTronApprovalTxConfirmation(amount, owner, spender, contractAddress, tronWeb, options = {}) {
    const { timeout = 120000, interval = 3000 } = options;
    const startTime = Date.now();
    logger$b.debug('Waiting for confirmation of tron approval tx');
    const input = viem.encodeFunctionData({
        abi: caCommon.ERC20ABI,
        functionName: 'allowance',
        args: [owner, spender],
    ...
    while (Date.now() - startTime < timeout) {
        try {
            const result = await tronWeb.transactionBuilder.triggerConstantContract(tronWeb.utils.address.fromHex(contractAddress), '', {
                input,
            }, [], tronWeb.utils.address.fromHex(owner));
            logger$b.debug('waitForTronApprovalTxConfirmation', {
                result,
            });
            if (result.Error) {
                throw Errors.internal(result.Error);
            }
            const allowance = viem.hexToBigInt(`0x${result.constant_result[0]}`);
            if (allowance < amount) {
...
const createExplorerTxURL = (txHash, explorerURL) => {
    return new URL(`/tx/${txHash}`, explorerURL).href;
};
const retrieveAddress = (universe, input) => {
    if (universe === caCommon.Universe.ETHEREUM) {
        return input.evm.address;
    }
    else if (universe === caCommon.Universe.TRON) {
        if (!input.tron) {
            throw Errors.internal('tron source but no tron input');
        }
        return input.tron.address;
    }
    throw Errors.internal('unknown universe');
};
const SIWE_KEY = '_siwe_sig';
const storeSIWESignatureToLocalStorage = (address, siweChain, signature) => {
    storageSetItem(`${SIWE_KEY}-${address}-${siweChain}`, signature);
};
const retrieveSIWESignatureFromLocalStorage = (address, siweChain) => {
    return storageGetItem(`${SIWE_KEY}-${address}-${siweChain}`);
};
const createDeadlineFromNow = (minutes = 3n) => {
...
const createCosmosQueryClient = async ({ cosmosRestUrl, cosmosGrpcWebUrl, cosmosWsUrl, }) => {
    // :| smh
    const rpc = await caCommon.createGrpcWebImpl(cosmosGrpcWebUrl);
    const cosmosQueryClient = new caCommon.QueryClientImpl(rpc);
    return {
        fetchMyIntents: async (address, page = 1) => {
            try {
                ...
                logger$a.error('Failed to fetch intents', error);
        ...
        fetchProtocolFees: async () => {
            try {
                ...
                logger$a.error('Failed to fetch protocol fees', error);
        ...
        fetchSolverData: async () => {
            try {
                ...
                logger$a.error('Failed to fetch solver data', error);
        ...
        waitForCosmosFillEvent: async (intentID, ac) => {
            ...
            try {
                ...
                for await (const resp of connection.source) {
                    logger$a.debug('waitForCosmosFillEvent', {
                        resp,
                    });
...
const intentTransform = (input, explorerBaseURL, chainList) => {
    return input.map((rff) => {
        const dstChainId = viem.bytesToNumber(rff.destinationChainID);
        ...
        return {
            ...
            destinationChain: {
                ...
                universe: caCommon.Universe[rff.destinationUniverse],
            },
            destinations: rff.destinations.map((d) => {
                const contractAddress = convertToHexAddressByUniverse(d.contractAddress, rff.destinationUniverse);
                ...
                const valueRaw = viem.bytesToBigInt(d.value);
                return {
                    token: {
            ...
            sources: rff.sources.map((s) => {
                const chainId = viem.bytesToNumber(s.chainID);
                ...
                const valueRaw = viem.bytesToBigInt(s.value);
                return {
                    chain: {
                        id: result.chain.id,
...
const getCoinbasePrices = async () => {
    if (coinbasePrices.lastUpdatedAt + COINBASE_UPDATE_INTERVAL < Date.now()) {
        try {
            const exchange = await axios__default.default.get('https://api.coinbase.com/v2/exchange-rates?currency=USD');
            coinbasePrices.rates = exchange.data.data.rates;
            coinbasePrices.lastUpdatedAt = Date.now();
        }
        catch (error) {
            logger$a.error('Failed to fetch Coinbase prices', error, { cause: 'INTERNAL_ERROR' });
...
const getFeeStore = async (cosmosQueryClient) => {
    const feeData = {
        fee: {
            collection: [],
            fulfilment: [],
...
const createVSCClient = ({ vscWsUrl, vscUrl }) => {
    ...
    return {
        getEVMBalancesForAddress: async (address) => {
            return getBalancesFromVSC(instance, address);
        },
        getTronBalancesForAddress: async (address) => {
            return getBalancesFromVSC(instance, address, 'TRON');
        },
        vscCreateFeeGrant: async (address) => {
            const response = await instance.post('/create-feegrant', {
                cosmos_address: address,
            });
            return response;
        },
        vscPublishRFF: async (id) => {
            const response = await instance.post('/publish-rff', {
                id: id.toNumber(),
            });
            logger$a.debug('publishRFF', { response });
            return { id };
        },
        vscCreateSponsoredApprovals: async (input) => {
            const connection = connect(new URL('/api/v1/create-sponsored-approvals', vscWsUrl).toString());
            await connection.connected();
            ...
            try {
                ...
                for await (const resp of connection.source) {
                    ...
                    logger$a.debug('vscCreateSponsoredApprovals', { data });
                    if ('errored' in data && data.errored) {
                        throw Errors.vscError(`failed to create sponsored approvals: ${data.msg ?? 'Backend sent failure.'}`);
                    }
                    ...
                    approvalHashes.push({
                        chainId: viem.bytesToNumber(inputData.chain_id),
                        hash: viem.toHex(data.tx_hash),
                    });
        ...
        vscCreateRFF: async (id, msd, expectedCollections) => {
            ...
            await esToolkit.retry(async () => {
                ...
                try {
                    ...
                    responseLoop: for await (const resp of connection.source) {
                        const data = msgpackr.unpack(resp);
                        logger$a.debug('vscCreateRFF:response', { data });
                        if ('idx' in data) {
                            // local msg
                            switch (data.status) {
                                ...
                                case 0xff: {
                                    if (pendingCollections.length === 0) {
                                        ...
                                        logger$a.debug('(vsc)create-rff:collections failed', {
                                            expectedCollections,
                                            completedCollections,
                                        });
                                        throw Errors.vscError(`create-rff: collections failed. expected = ${expectedCollections}, got = ${completedCollections}`);
                                    }
                                }
                                // Collection successful for a chain
                                case 0x10: {
                                    const pendingCollection = pendingCollections.find((pc) => pc.index === data.idx);
                                    if (pendingCollection) {
                                        completedCollections.push(data.idx);
                                        esToolkit.remove(pendingCollections, (d) => d.index === data.idx);
                                        msd({
                                            current: completedCollections.length,
                                            total: expectedCollections.length,
                                            txHash: viem.toHex(data.tx_hash),
                                ...
                                default: {
                                    if (pendingCollections.find((pc) => pc.index === data.idx)) {
                                        logger$a.debug('vsc:create-rff:failed', { data });
                                    }
                                    else {
                                        logger$a.debug('vsc:create-rff:expectedError:ignore', { data });
                                    }
                                }
                            }
                        }
...
const getBalancesFromVSC = async (instance, address, namespace = 'ETHEREUM') => {
    const response = await instance.get(`/get-balance/${namespace}/${address}`);
    logger$a.debug('getBalancesFromVSC', { address, namespace, response });
    return response.data.balances.filter((b) => b.errored !== true);
};

const EADDRESS = '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';
const EADDRESS_32_BYTES = convertTo32Bytes$1(EADDRESS);
...
const createBatchedCallSignature = (batchedCalls, nonce, chain, address, account, deadline) => {
...
const waitForSBCTxReceipt = (ops, chainList, publicClientList) => {
    return Promise.all(ops.map((op) => {
        ...
        logger$9.debug('waitForSBCTxReceipt', { explorerURL: explorerURL.toString() });
...
const createSBCTxFromCalls = async ({ cache, calls, chainID, ephemeralAddress, ephemeralWallet, publicClient, }) => {
    const nonce = viem.bytesToBigInt(await cryptoGetRandomValues(new Uint8Array(24))) << 64n;
    const deadline = createDeadlineFromNow(3n);
    const signature = await createBatchedCallSignature(calls, nonce, BigInt(chainID), ephemeralAddress, ephemeralWallet, deadline);
    ...
    logger$9.debug('createSBCTxFromCalls', {
        request,
    });
    return request;
};
const caliburExecute = async ({ actualAddress, actualWallet, calls, chain, ephemeralAddress, ephemeralWallet, value, }) => {
    const nonce = viem.bytesToBigInt(await cryptoGetRandomValues(new Uint8Array(24))) << 64n;
    const deadline = createDeadlineFromNow(3n);
    const signature = await createBatchedCallSignature(calls, nonce, BigInt(chain.id), ephemeralAddress, ephemeralWallet, deadline);
...
const convertToEVMAddress = (addr) => {
    let address = addr;
...
const createPermitSignature = async (contractAddress, client, spender, walletAddress, variant, version, deadline, amount) => {
    const contract = viem.getContract({
        abi: caCommon.ERC20ABI,
        address: contractAddress,
        client,
    ...
    logger$8.debug('createPermitSigParams', {
...
const isAuthorizationCodeSet = async (chainID, address, cache) => {
    ...
    logger$8.debug('isAuthorizationCodeSet', { code, EXPECTED_CALIBUR_CODE });
    if (!code) {
        return false;
    }
    return code !== '0x' && equalFold(code, EXPECTED_CALIBUR_CODE);
};
const isNativeAddress$1 = (contractAddress) => equalFold(contractAddress, ZERO_ADDRESS) || equalFold(contractAddress, EADDRESS);
/**
...
const createPermitAndTransferFromTx = async ({ amount, approval, cache, chain, contractAddress, owner, ownerWallet, publicClient, spender, }) => {
    ...
    logger$8.debug('createPermitCalls', {
        contractAddress,
        EADDRESS,
    });
    ...
    if (allowance === undefined) {
        logger$8.debug('createPermitCalls: allowance not found in cache', {
            cache,
            chain,
            contractAddress,
            owner,
    ...
    logger$8.debug('createPermitTx', { allowance, amount });
    if (allowance < amount) {
...
const determinePermitVariantAndVersion = async (client, contractAddress) => {
    const standardPermitData = viem.encodeFunctionData({
        abi: [
            {
                type: 'function',
                name: 'permit',
    ...
    const daiPermitData = viem.encodeFunctionData({
        abi: [
            {
                type: 'function',
                name: 'permit',
    ...
    const [canonicalPermitResponse, daiPermitResponse, versionResponse] = await Promise.allSettled(promises);
    let variant = caCommon.PermitVariant.Unsupported;
    if (canonicalPermitResponse.status === 'fulfilled') {
        variant = caCommon.PermitVariant.EIP2612Canonical;
    }
    else if (daiPermitResponse.status === 'fulfilled') {
        variant = caCommon.PermitVariant.DAI;
    }
    return {
        variant,
...
async function getVersion(client, token) {
    try {
...
async function functionExists(client, token, data) {
    return client.call({ to: token, data });
}
const createPermitApprovalTx = async ({ contractAddress, owner, ownerWallet, spender, variant, version, amount, }) => {
    const deadline = createDeadlineFromNow(3n);
    const { signature } = await createPermitSignature(contractAddress, ownerWallet, spender, owner, variant, version, deadline, amount);
...
const packERC20Approve = (spender, amount) => {
...
const getAnkrBalances = async (walletAddress, chainList, removeTransferFee) => {
    const publicClients = {};
    const res = await axios__default.default.post('https://rpcs.avail.so/multichain', {
    ...
    if (!res.data?.result)
        throw Errors.internal('balances cannot be retrieved');
    ...
    for (const asset of filteredAssets) {
        promises.push((async () => {
            ...
            if (asset.tokenType === 'NATIVE') {
                ...
                logger$8.debug('getAnkrBalances', {
...
const toFlatBalance = (assets, convertAddressToBytes32 = true, currentChainID, selectedTokenAddress) => {
    logger$8.debug('toFlatBalance', {
        assets,
    });
    return assets
        .flatMap((a) => a.breakdown.map((b) => {
...
const vscBalancesToAssets = (chainList, evmBalances = [], tronBalances = []) => {
    const assets = [];
    const vscBalances = evmBalances.concat(tronBalances);
    logger$8.debug('balanceToAssets', {
        evmBalances,
        tronBalances,
    });
...
const ankrBalanceToAssets = (chainList, ankrBalances, filterWithSupportedTokens, allowedSources = [], removeSources = []) => {
    const assets = [];
    const allowed = (chainId, tokenAddress) => {
        if (allowedSources.length === 0) {
            return true;
        }
        return !!allowedSources.find((s) => s.chainId === chainId && equalFold(s.tokenAddress, tokenAddress));
    };
    for (const asset of ankrBalances) {
        ...
        const isSupportedToken = isTokenSupported(asset.chainID, convertTo32BytesHex(asset.tokenAddress));
        ...
        const removeSource = !!removeSources.find((rs) => rs.chainId === asset.chainID && equalFold(rs.tokenAddress, asset.tokenAddress));
        logger$8.debug('ankrBalanaceToAssets', {
            isAllowed,
            isSupportedToken,
            allowedSources,
        ...
        if (existingAsset) {
            if (!existingAsset.breakdown.some((t) => t.chain.id === chain.id && equalFold(t.contractAddress, asset.tokenAddress))) {
...
class Cache {
    publicClientList;
    allowanceValues = new Map();
    setCodeValues = new Map();
    allowanceQueries = new Set();
    nativeAllowanceQueries = new Set();
    setCodeQueries = new Set();
...
const parseQuote = (input, createApproval = true) => {
    logger$8.debug('getTxsFromQuote', {
        createApproval,
        input,
    });
    if (input.agg instanceof caCommon.LiFiAggregator) {
        const originalResponse = input.quote.originalResponse;
        ...
        if (createApproval) {
            val.swap.approval = {
                data: packERC20Approve(originalResponse.estimate.approvalAddress, input.quote.inputAmount),
    ...
    else if (input.agg instanceof caCommon.BebopAggregator) {
        const originalResponse = input.quote.originalResponse;
        ...
        if (createApproval) {
            val.swap.approval = {
                data: packERC20Approve(originalResponse.quote.approvalTarget, input.quote.inputAmount),
                to: convertToEVMAddress(input.inputToken),
                value: 0n,
            };
        }
...
const getTokenInfo = async (contractAddress, publicClient, chain) => {
    if (isNativeAddress$1(contractAddress)) {
        return {
            contractAddress: ZERO_ADDRESS,
            decimals: chain.nativeCurrency.decimals,
            symbol: chain.nativeCurrency.symbol,
...
caCommon.msgpackableAxios.create({
...
const createSweeperTxs = ({ cache, chainID, COTCurrencyID, receiver, sender, tokenAddress, }) => {
    ...
    if (isNativeAddress$1(tokenAddress)) {
        ...
        logger$8.debug('createSweeperTxs', {
            nativeAllowance,
        });
        ...
        if (!sweeperAllowance || sweeperAllowance === 0n) {
            txs.push({
                data: packERC20Approve(SWEEPER_ADDRESS, viem.maxUint256),
...
const performDestinationSwap = async ({ actualAddress, cache, calls, chain, chainList, COT, emitter, ephemeralAddress, ephemeralWallet, hasDestinationSwap, publicClientList, vscClient, }) => {
    try {
        ...
        logger$8.error('destination swap failed twice, sweeping to eoa', e, { cause: 'SWAP_FAILED' });
        await vscClient
            .vscSBCTx([
            ...
            .catch((e) => {
            logger$8.error('error during destination sweep', e, { cause: 'DESTINATION_SWEEP_ERROR' });
        });
        throw e;
    }
};
...
const sortSourcesByPriority = (balances, destination) => {
    const STABLECOINS = ['USDC', 'USDT', 'DAI'];
    const isGasToken = (tokenAddress) => {
        return (equalFold(tokenAddress, convertTo32BytesHex(ZERO_ADDRESS)) ||
            equalFold(tokenAddress, convertTo32BytesHex(EADDRESS)));
    };
    const isSameToken = (balance) => {
        return (equalFold(balance.tokenAddress, convertTo32BytesHex(destination.tokenAddress)) ||
            (balance.symbol === destination.symbol &&
                balance.symbol !== 'ETH' &&
                balance.symbol !== 'WETH'));
    };
    const getPriority = (balance) => {
        // Priority levels:
        ...
        logger$8.debug('sortSourcesByPriority:getPriority', {
            isSame,
            isSameChain,
            isEthereum,
            isGas,
        ...
        if (isSameChain) {
            if (isSame)
                return 1;
            if (!isEthereum) {
                if (STABLECOINS.some((coin) => equalFold(coin, balance.symbol)))
                    return 2;
                if (isGas)
                    return 3;
                return 4;
    ...
    return esToolkit.orderBy(balances, [getPriority, (b) => b.value], ['asc', 'desc']);
};
...
(function (CurrencyID) {
    ...
    CurrencyID[CurrencyID["DAI"] = 8] = "DAI";
...
const isTokenSupported = (chainId, contractAddress) => {
...
const getPermitVariantAndVersion = async (tokenAddress, client) => {
...
const getTokenDecimals = (chainID, contractAddress) => {
    ...
    if (!token) {
        throw Errors.assetNotFound(utils.toHex(contractAddress));
    }
    return {
...
const getBalancesForSwap = async (input) => {
    const ankrBalances = await getAnkrBalances(input.evmAddress, input.chainList);
    const assets = ankrBalanceToAssets(input.chainList, ankrBalances, input.filterWithSupportedTokens, input.allowedSources, input.removeSources);
    let balances = toFlatBalance(assets);
    logger$e.debug('getBalancesForSwap', {
        input,
        ankrBalances,
        assets,
        balances,
...
const getBalanceSlot = ({ tokenSymbol, chainId, userAddress, }) => {
    ...
    const userBalanceSlot = viem.keccak256(viem.encodePacked(['bytes32', 'uint256'], [viem.pad(userAddress, { size: 32 }), BigInt(balanceSlot)]));
    logger$e.debug('getBalanceSlot', {
        tokenSymbol,
        chainId,
        userAddress,
...
const generateStateOverride = (params) => {
    const amountInHex = viem.toHex(params.amount * 2n);
    ...
    return {
        [params.tokenAddress]: {
            storage: {
                [balanceSlot]: viem.pad(amountInHex, { size: 32 }),
            },
        },
        [params.userAddress]: {
            balance: viem.toHex(100000n),
        },
    };
};
const DEFAULT_SLOT = {
...
function getBalanceStorageSlot(token, chainId) {
    ...
    logger$e.debug('storageSlotMapping', {
        storageSlotMapping,
        chainId,
    ...
    if (chainMapping) {
        ...
        if (slot) {
            logger$e.info(`Using storage slot ${slot} for ${token} on chain ${chainId}`);
            return slot;
        }
    }
    logger$e.warn(`Unsupported chain ${chainId}, falling back to defaults`);
...
function tronHexToEvmAddress(tronHex) {
...
const getSourcesAndDestinationsForRFF = (intent, chainList, _) => {
    const sources = [];
    ...
    for (const source of intent.sources) {
        ...
        if (!token) {
            logger$7.error('Token not found', { source });
...
const createRFFromIntent = async (intent, options, destinationUniverse) => {
    const { destinations, sources, universes } = getSourcesAndDestinationsForRFF(intent, options.chainList);
    const parties = [];
    for (const universe of universes) {
        ...
        if (universe === caCommon.Universe.TRON) {
            logger$7.debug('createRFFFromIntent: Tron', {
                tronAddress: tronweb.TronWeb.address.toHex(options.tron.address),
            });
            parties.push({
                address: convertTo32BytesHex(tronHexToEvmAddress(tronweb.TronWeb.address.toHex(options.tron.address))),
                universe,
            });
        }
    }
    logger$7.debug('processRFF:1', {
        destinations,
        parties,
        sources,
        universes,
    ...
    const omniversalRFF = new caCommon.OmniversalRFF({
        ...
        destinations: destinations.map((dest) => ({
            contractAddress: viem.toBytes(dest.tokenAddress),
            value: viem.toBytes(dest.value),
        })),
        recipientAddress: convertTo32Bytes$1(intent.recipientAddress),
        ...
        nonce: await cryptoGetRandomValues(new Uint8Array(32)),
        // @ts-expect-error
        ...
        sources: sources.map((source) => ({
            ...
            contractAddress: convertTo32Bytes$1(source.tokenAddress),
            universe: source.universe,
            value: viem.toBytes(source.valueRaw),
        })),
    });
    const signatureData = [];
    for (const universe of universes) {
        ...
        if (universe === caCommon.Universe.TRON) {
            if (!options.tron) {
                logger$7.error('universe has tron but not expected input', {
                    tronInput: options.tron,
                });
                throw Errors.internal('universe has tron but not expected input');
            }
            const { requestHash, signature } = await createRequestTronSignature(omniversalRFF.asEVMRFF(), options.tron.adapter);
            signatureData.push({
                address: convertTo32Bytes$1(tronHexToEvmAddress(tronweb.TronWeb.address.toHex(options.tron.address))),
                requestHash,
                signature,
                universe,
            });
...
const calculateMaxBridgeFee = ({ assets, feeStore, dst, }) => {
    ...
    logger$7.debug('calculateMaxBridgeFees:1', {
    ...
    for (const asset of assets) {
        ...
        logger$7.debug('calculateMaxBridgeFees:2', {
            borrow: borrow.toFixed(),
            borrowWithFee: borrowWithFee.toFixed(),
    ...
    const maxAmount = fee.lt(borrow)
        ? borrow.minus(fee).toFixed(dst.decimals, Decimal__default.default.ROUND_FLOOR)
        : '0';
...
const isNativeAddress = (universe, address) => {
    if (universe === caCommon.Universe.ETHEREUM || universe === caCommon.Universe.TRON) {
...
class ChainList {
    ...
    getChainAndTokenByAddress(chainID, address) {
        const chain = this.getChainByID(chainID);
        if (!chain) {
            return undefined;
        }
    ...
    getChainAndTokenFromSymbol(chainID, tokenSymbol) {
        const chain = this.getChainByID(chainID);
        if (!chain) {
            throw Errors.chainNotFound(chainID);
        }
 ...
 */
const getGasPriceRecommendations = async (publicClient) => {
    try {
        ...
        logger$6.debug('Gas price recommendations', {
        ...
        logger$6.error('Failed to get gas price recommendations', { error });
...
class BridgeAndExecuteQuery {
    ...
    getUnifiedBalances;
    simulationClient;
    constructor(chainList, evmClient, bridge, getUnifiedBalances, simulationClient) {
        this.chainList = chainList;
        this.evmClient = evmClient;
        this.bridge = bridge;
        this.getUnifiedBalances = getUnifiedBalances;
        this.simulationClient = simulationClient;
    }
    async estimateBridgeAndExecute(params) {
        const { toChainId, token: tokenSymbol, amount, execute } = params;
        const { token, chain: dstChain } = this.chainList.getChainAndTokenFromSymbol(params.toChainId, tokenSymbol);
        if (!token) {
            throw Errors.tokenNotFound(tokenSymbol, toChainId);
        }
        const address = (await this.evmClient.getAddresses())[0];
        const txs = [];
        const { tx, approvalTx, dstPublicClient } = await this.createTxsForExecute({ ...execute, toChainId: params.toChainId }, address);
        logger$e.debug('BridgeAndExecute:2', {
            tx,
            dstChain,
            dstPublicClient,
            approvalTx,
        ...
        const determineGasUsed = params.execute.gas
            ? Promise.resolve({ approvalGas: approvalTx ? 70000n : 0n, txGas: params.execute.gas })
            : this.simulateBundle({
                txs,
        ...
        const [gasUsed, gasPriceRecommendations, balances, l1Fee] = await Promise.all([
            ...
            getL1Fee(execute.to, dstChain, viem.serializeTransaction({
                chainId: dstChain.id,
        ...
        const [suggestedApprovalGas, approvalGas] = pctAdditionWithSuggestion(gasUsed.approvalGas, pctBuffer);
        ...
        logger$e.debug('BridgeAndExecute:3', {
        ...
        const { skipBridge, tokenAmount, gasAmount } = await this.calculateOptimalBridgeAmount(dstChain, token.contractAddress, token.decimals, amount, gasFee, balances);
        return {
            dstPublicClient,
            dstChain,
            amount: {
    ...
    async simulateBridgeAndExecute(params) {
        const { gasFee, token, skipBridge, amount, gas, gasPrice } = await this.estimateBridgeAndExecute(params);
        logger$e.debug('BridgeAndExecute:4:CalculateOptimalBridgeAmount', {
            skipBridge,
            amount,
            gas,
        });
     ...
     */
    async bridgeAndExecute(params, options) {
        const { dstPublicClient, dstChain, address, token, skipBridge, tx, approvalTx, amount, gas, gasPrice, } = await this.estimateBridgeAndExecute(params);
        logger$e.debug('BridgeAndExecute:4:CalculateOptimalBridgeAmount', {
            params,
            skipBridge,
            amount,
        ...
        if (options?.beforeExecute) {
            ...
            logger$e.debug('BeforeExecuteHook', {
                response,
            });
            if (response.data) {
        ...
        logger$e.debug('BridgeAndExecute:5', {
            executeResponse,
        });
        // 9. Return result
        const result = {
    ...
    async createTxsForExecute(params, address) {
        ...
        const dstPublicClient = viem.createPublicClient({
            transport: viem.http(dstChain.rpcUrls.default.http[0]),
        });
        // 2. Check if token is supported
        let approvalTx = null;
        if (params.tokenApproval) {
            const token = this.chainList.getTokenInfoBySymbol(params.toChainId, params.tokenApproval.token);
            if (!token) {
    ...
    async execute(params, options) {
        const address = (await this.evmClient.getAddresses())[0];
        const { dstPublicClient, dstChain, approvalTx, tx } = await this.createTxsForExecute(params, address);
    ...
    async simulateExecute(params, address) {
     ...
     */
    async calculateOptimalBridgeAmount(chain, tokenAddress, tokenDecimals, requiredTokenAmount, requiredGasAmount, assets) {
        let skipBridge = true;
        let tokenAmount = requiredTokenAmount;
        let gasAmount = requiredGasAmount;
        const assetList = new UserAssets(assets);
        const { destinationAssetBalance, destinationGasBalance } = assetList.getAssetDetails(chain, tokenAddress);
        ...
        logger$e.debug('calculateOptimalBridgeAmount', {
            destinationTokenAmount,
            requiredTokenAmount,
            destinationGasAmount,
            requiredGasAmount,
        });
        if (isNativeAddress(caCommon.Universe.ETHEREUM, tokenAddress)) {
    ...
    async sendTx(params, options) {
        ...
        let receipt;
        if (waitForReceipt) {
            receipt = await waitForTxReceipt(txHash, options.dstPublicClient, requiredConfirmations, receiptTimeout);
            if (options.emit) {
                options.emit({
                    name: NEXUS_EVENTS.STEP_COMPLETE,
    ...
    bridgeWrapper = async (params, options) => {
    ...
    simulateBridgeWrapper = async (params) => {
...
const createBridgeAndTransferParams = (input, chainList) => {
...
class SwapAndExecuteQuery {
    ...
    getBalancesForSwap;
    swap;
    constructor(chainList, evmClient, getBalancesForSwap, swap) {
        this.chainList = chainList;
        this.evmClient = evmClient;
        this.getBalancesForSwap = getBalancesForSwap;
        this.swap = swap;
    }
    async estimateSwapAndExecute(params) {
        const { toChainId, toAmount, execute } = params;
        const address = (await this.evmClient.getAddresses())[0];
        const { tx, approvalTx, dstChain, dstPublicClient } = await this.createTxsForExecute(execute, toChainId, address);
        logger$e.debug('SwapAndExecute:2', {
            tx,
            dstPublicClient,
            approvalTx,
        });
        const determineGasUsed = Promise.resolve({
            approvalGas: approvalTx ? 70000n : 0n,
        ...
        tx.gas = suggestedTxGas;
        const gasFee = (approvalGas + txGas) * gasPrice + l1Fee;
        logger$e.debug('SwapAndExecute:3', {
            increasedGas: approvalGas + txGas,
            approvalGas,
            txGas,
            gasPriceRecommendations,
        ...
        // 6. Determine gas or token needed via bridge
        const { skipSwap, tokenAmount, gasAmount } = await this.calculateOptimalSwapAmount(toChainId, dstTokenInfo.contractAddress, dstTokenInfo.decimals, toAmount, gasFee, balances.balances);
        return {
            dstPublicClient,
            dstChain,
            amount: {
    ...
    async createTxsForExecute(params, chainId, address) {
        // 1. Check if dst chain data is available
        ...
        if (params.tokenApproval) {
            ...
            const requiredAllowance = BigInt(params.tokenApproval.amount);
            logger$e.debug('SwapAndExecute:createTxsForExecute', {
                requiredAllowance,
                currentAllowance,
    ...
    async swapAndExecute(params, options) {
        const { dstPublicClient, dstChain, address, skipSwap, tx, approvalTx, amount, gas, gasPrice, dstTokenInfo, gasFee, } = await this.estimateSwapAndExecute(params);
        logger$e.debug('BridgeAndExecute:4:CalculateOptimalSwapAmount', {
            params,
            skipSwap,
        ...
        if (skipSwap) {
            ...
            logger$e.debug('swapResult:SwapAndExecute()', { swapResult });
        }
        const executeResponse = await this.sendTx({
            approvalTx,
            tx,
        ...
        logger$e.debug('swapResult:executeResponse()', { executeResponse });
        const result = {
            swapResult,
            swapSkipped: skipSwap,
            executeResponse,
        };
        logger$e.debug('swapResult:full result', { result });
        return result;
    }
    /**
     * Calculate optimal bridge amount based on destination chain balance
     * Returns the exact amount needed to bridge, or indicates if bridge can be skipped entirely
     */
    async calculateOptimalSwapAmount(toChainId, tokenAddress, tokenDecimals, requiredTokenAmount, requiredGasAmount, balances) {
        let skipSwap = true;
        let tokenAmount = requiredTokenAmount;
        let gasAmount = requiredGasAmount;
        let destinationTokenAmount = 0n;
        const tokenBalance = balances.find((b) => b.chainID === toChainId && equalFold(b.tokenAddress, convertTo32BytesHex(tokenAddress)));
        if (tokenBalance) {
            destinationTokenAmount = mulDecimals(tokenBalance.amount, tokenDecimals);
        }
        let destinationGasAmount = 0n;
        const gasBalance = balances.find((b) => b.chainID === toChainId && equalFold(b.tokenAddress, convertTo32BytesHex(EADDRESS)));
        if (gasBalance) {
...
const createSteps = (intent, chainList, unallowedSources) => {
    ...
    if (unallowedSources && unallowedSources?.length > 0) {
        for (const source of unallowedSources) {
            steps.push(BRIDGE_STEPS.ALLOWANCE_APPROVAL_REQUEST(source.chain), BRIDGE_STEPS.ALLOWANCE_APPROVAL_MINED(source.chain));
        }
...
let BridgeHandler$1 = class BridgeHandler {
    options;
    steps = [];
    params;
    ...
    buildIntent = async (sourceChains = []) => {
        ...
        logger$5.debug('Step 0: BuildIntent', {
            assets,
            oraclePrices,
            feeStore,
        });
        console.timeEnd('preIntentSteps:API');
        logger$5.debug('Step 1:', {
            assets,
            feeStore,
            oraclePrices,
        });
        ...
        const nativeAmountInDecimal = divDecimals(this.params.nativeAmount, this.params.dstChain.nativeCurrency.decimals);
        const tokenAmountInDecimal = divDecimals(this.params.tokenAmount, this.params.dstToken.decimals);
        const gasInToken = convertGasToToken(this.params.dstToken, oraclePrices, this.params.dstChain.id, this.params.dstChain.universe, nativeAmountInDecimal);
        console.timeEnd('preIntentSteps: CalculateGas');
        logger$5.debug('preIntent:1', {
    ...
    filterInsufficientAllowanceSources(intent, allowances) {
        const sources = [];
        for (const s of intent.sources) {
            ...
            logger$5.debug('getUnallowedSources:1', {
    ...
    execute = async (shouldRetryOnFailure = true) => {
        try {
            ...
            const refresh = async (sourceChains) => {
                if (accepted) {
                    logger$5.warn('Intent refresh called after acceptance');
            ...
            logger$5.debug('intent', { intent });
            const response = await this.processRFF(intent);
            if (response.retry) {
                logger$5.debug('rff fee expired, going to rebuild intent...');
            ...
            if (this.params.dstChain.universe === caCommon.Universe.ETHEREUM) {
            ...
            logger$5.error('bridge: execute error', error);
            throw error;
        }
    };
    async waitForFill(requestHash, intentID, waitForDoubleCheckTx) {
        ...
        if (this.params.dstChain.universe === caCommon.Universe.ETHEREUM) {
            promisesToRace.push(waitForIntentFulfilment(viem.createPublicClient({
                transport: viem.webSocket(this.params.dstChain.rpcUrls.default.webSocket[0]),
            }), this.options.chainList.getVaultContractAddress(this.params.dstChain.id), requestHash, ac));
        }
        await Promise.race(promisesToRace);
        logger$5.debug('Fill completed');
    }
    async processRFF(intent) {
        const { msgBasicCosmos, omniversalRFF, signatureData, sources, universes } = await createRFFromIntent(intent, this.options);
        this.markStepDone(BRIDGE_STEPS.INTENT_HASH_SIGNED);
        logger$5.debug('processRFF:3', { msgBasicCosmos });
        ...
        const tronSignatureData = signatureData.find((d) => d.universe === caCommon.Universe.TRON);
        if (!tronSignatureData && universes.has(caCommon.Universe.TRON)) {
        ...
        for (const [i, s] of sources.entries()) {
            ...
            if (s.universe === caCommon.Universe.ETHEREUM && isNativeAddress(s.universe, s.tokenAddress)) {
            ...
            else if (s.universe === caCommon.Universe.TRON) {
                const provider = new tronweb.TronWeb({
                    fullHost: chain.rpcUrls.default.grpc[0],
                });
                const vaultContractAddress = this.options.chainList.getVaultContractAddress(Number(s.chainID));
                const txWrap = await provider.transactionBuilder.triggerSmartContract(tronweb.TronWeb.address.fromHex(vaultContractAddress), '', {
                    txLocal: true,
                    input: viem.encodeFunctionData({
                        abi: caCommon.EVMVaultABI,
                        functionName: 'deposit',
                        args: [omniversalRFF.asEVMRFF(), viem.toHex(tronSignatureData.signature), BigInt(i)],
                    }),
                }, [], tronweb.TronWeb.address.fromHex(this.options.tron.address));
                const signedTx = await this.options.tron.adapter.signTransaction(txWrap.transaction);
                logger$5.debug('tron deposit signTransaction result', {
                    signedTx,
                });
                if (!this.options.tron.adapter.isMobile) {
                    const txResult = await provider.trx.sendRawTransaction(signedTx);
                    logger$5.debug('tron deposit tx result', {
                        txResult,
                    });
                    if (!txResult.result) {
                ...
                tronDeposits.push((async () => {
                    await waitForTronDepositTxConfirmation(tronSignatureData.requestHash, vaultContractAddress, provider, this.options.tron.address);
                })());
            }
            doubleCheckTxs.push(createDepositDoubleCheckTx(convertTo32Bytes$1(chain.id), this.options.cosmos, intentID));
        }
        ...
        logger$5.debug('PostIntentSubmission: Intent ID', {
        ...
        if (tokenCollections.length > 0) {
            const markCollectionDone = (params) => {
                ...
                if (!chain) {
                    logger$5.debug('MarkCollectionDone: chain not found?', {
                ...
                this.markStepDone(BRIDGE_STEPS.INTENT_COLLECTION(params.current, params.total, params.txHash, explorerUrl));
            };
            logger$5.debug('processRFF', {
    ...
    async setAllowances(input) {
        ...
        logger$5.debug('setAllowances', { originalChain, input });
        const sponsoredApprovals = [];
        const unsponsoredApprovals = [];
        try {
            for (const source of input) {
                ...
                const chainId = new caCommon.OmniversalChainID(chain.universe, source.chainID);
                ...
                if (chain.universe === caCommon.Universe.ETHEREUM) {
                    logger$5.debug(`Switching chain to ${chain.id}`);
                    await switchChain(this.options.evm.client, chain);
                }
                if (currency.permitVariant === caCommon.PermitVariant.Unsupported || chain.id === 1) {
                    if (chain.universe === caCommon.Universe.ETHEREUM) {
                        const h = await this.options.evm.client
                            ...
                            if (e instanceof viem.ContractFunctionExecutionError) {
                                const isUserRejectedRequestError = e.walk((e) => e instanceof viem.UserRejectedRequestError) instanceof
                    ...
                    else if (chain.universe === caCommon.Universe.TRON) {
                        ...
                        const tx = await provider.transactionBuilder.triggerSmartContract(tronweb.TronWeb.address.fromHex(source.tokenContract), 'approve(address,uint256)', {
                            txLocal: true,
                        }, [
                            { type: 'address', value: tronweb.TronWeb.address.fromHex(vc) },
                            { type: 'uint256', value: source.amount.toString() },
                        ], tronweb.TronWeb.address.fromHex(this.options.tron?.address));
                        const signedTx = await this.options.tron.adapter.signTransaction(tx.transaction);
                        logger$5.debug('tron approval signTransaction result', {
                            signedTx,
                        });
                        if (!this.options.tron.adapter.isMobile) {
                            const txResult = await provider.trx.sendRawTransaction(signedTx);
                            logger$5.debug('tron tx result', {
                                txResult,
                            });
                            if (!txResult.result) {
                        ...
                        await waitForTronApprovalTxConfirmation(source.amount, this.options.tron.address, vc, source.tokenContract, provider);
                    }
                    ...
                    const signed = viem.parseSignature(await signPermitForAddressAndValue(currency, this.options.evm.client, publicClient, account, vc, source.amount).catch((e) => {
                        if (e instanceof viem.ContractFunctionExecutionError) {
                            const isUserRejectedRequestError = e.walk((e) => e instanceof viem.UserRejectedRequestError) instanceof
                                viem.UserRejectedRequestError;
                            if (isUserRejectedRequestError) {
            ...
            if (sponsoredApprovals.length) {
                logger$5.debug('setAllowances:sponsoredApprovals', {
                    sponsoredApprovals,
                });
                const approvalHashes = await this.options.vscClient.vscCreateSponsoredApprovals(sponsoredApprovals);
                await Promise.all(approvalHashes.map(async (approval) => {
            ...
            logger$5.error('Error setting allowances', e, { cause: 'ALLOWANCE_SETTING_ERROR' });
            throw e;
        }
        finally {
            if (this.params.dstChain.universe === caCommon.Universe.ETHEREUM) {
    ...
    async waitForOnAllowanceHook(sources) {
        ...
        await new Promise((resolve, reject) => {
            const allow = (allowances) => {
                ...
                logger$5.debug('CA:BaseRequest:Allowances', {
                    allowances,
                    sources,
                });
                const val = [];
    ...
    createExpectedSteps(intent, insufficientAllowanceSources) {
        ...
        logger$5.debug('BridgeSteps', this.steps);
    }
    async createIntent(input) {
        ...
        const destinationBalance = asset.getBalanceOnChain(this.params.dstChain.id, token.contractAddress);
        const borrow = amount;
        ...
        logger$5.debug('createIntent:1', { fulfilmentFee });
        ...
        logger$5.debug('createIntent:1.1', { allowedSources });
        for (const assetC of allowedSources) {
            ...
            borrowWithFee = borrowWithFee.add(collectionFee);
            logger$5.debug('createIntent:2', { collectionFee });
            ...
            let borrowFromThisChain = new Decimal__default.default(assetC.balance).lessThanOrEqualTo(unaccountedAmount)
                ...
                : unaccountedAmount;
            logger$5.debug('createIntent:2.1', {
            ...
            logger$5.debug('createIntent:3', { solverFee });
            borrowWithFee = borrowWithFee.add(solverFee);
        ...
        if (accountedAmount.lt(borrowWithFee)) {
            throw Errors.insufficientBalance(`required: ${borrowWithFee.toFixed()} ${token.symbol}, available: ${accountedAmount.toFixed()} ${token.symbol}`);
        }
        if (!gas.equals(0)) {
            intent.destination.gas = mulDecimals(gas, this.params.dstChain.nativeCurrency.decimals);
        }
        logger$5.debug('createIntent:4', { intent });
        return intent;
    }
    markStepDone = (step) => {
        if (this.options.emit) {
...
const isDeposit = (universe, tokenAddress) => {
    if (universe === caCommon.Universe.ETHEREUM) {
...
const getMaxValueForBridge = async (params, options) => {
...
const createBridgeParams = (input, chainList) => {
    if (input.amount === 0n && (!input.gas || input.gas === 0n)) {
        throw Errors.invalidInput(`input.amount & input.gas can't be 0`);
    }
    const { chain: dstChain, token: dstToken } = chainList.getChainAndTokenFromSymbol(input.toChainId, input.token);
    if (!dstToken) {
        throw Errors.tokenNotFound(input.token, input.toChainId);
    }
    const params = {
...
const determineSwapRoute = async (input, options) => {
    logger$4.debug('determineSwapRoute', {
        input,
        options,
    });
    return input.mode === exports.SwapMode.EXACT_OUT
        ? _exactOutRoute(input.data, options)
        : _exactInRoute(input.data, options);
};
const applyBuffer = (amount, bufferPercent) => amount.mul(1 + bufferPercent / 100);
/*
Exact out:
sF = solver Fee
cF = collection Fee
...
var BUFFER_EXACT_OUT;
(function (BUFFER_EXACT_OUT) {
    BUFFER_EXACT_OUT[BUFFER_EXACT_OUT["DESTINATION_SWAP"] = 5] = "DESTINATION_SWAP";
    BUFFER_EXACT_OUT[BUFFER_EXACT_OUT["SOURCE_SWAP"] = 2] = "SOURCE_SWAP";
})(BUFFER_EXACT_OUT || (BUFFER_EXACT_OUT = {}));
const _exactOutRoute = async (input, params) => {
    ...
    logger$4.debug('determineSwapRoute', { balances, input });
    ...
    let gasInCOT = new Decimal__default.default(0);
    if (input.toNativeAmount && input.toNativeAmount > 0n) {
        ...
        }, oraclePrices, dstChain.id, dstChain.universe, divDecimals(input.toNativeAmount ?? 0n, dstChain.nativeCurrency.decimals)).mul(1.02);
    }
    ...
    const fetchDestinationSwapDetails = async () => {
        ...
        if (!equalFold(input.toTokenAddress, dstChainCOTAddress)) {
            const [ds, dgs] = await Promise.all([
                caCommon.determineDestinationSwaps(userAddressInBytes, dstOmniversalChainID, {
                    amount: BigInt(input.toAmount),
                    tokenAddress: convertTo32Bytes(input.toTokenAddress),
                }, params.aggregators),
                gasInCOT.isZero()
                    ? null
                    : caCommon.destinationSwapWithExactIn(userAddressInBytes, dstOmniversalChainID, mulDecimals(gasInCOT, dstChainCOT.decimals), EADDRESS_32_BYTES, params.aggregators),
            ]);
            destinationSwap = ds;
            if (dgs) {
                gasSwap = {
                    ...
                    originalHolding: {
                        ...
                        value: 0,
                        decimals: dstChainCOT.decimals,
                        symbol: caCommon.CurrencyID[dstChainCOT.currencyID],
                    },
                };
            }
        }
        ...
        const max = applyBuffer(min, BUFFER_EXACT_OUT.DESTINATION_SWAP).toDP(dstChainCOT.decimals, Decimal__default.default.ROUND_CEIL);
        return {
    ...
    logger$4.debug('destination swaps', destinationSwap);
    // Collection Fee needs to be calculated on cot
    const estimatedCollectionFee = estimateCollectionFee(esToolkit.uniqBy(balances, (b) => b.chainID).map((b) => {
        const cdm = caCommon.ChaindataMap.get(new caCommon.OmniversalChainID(caCommon.Universe.ETHEREUM, b.chainID));
        if (!cdm) {
            throw Errors.internal(`chain data not found for chain ${input.toChainId}`);
        }
    ...
    }), destinationSwap.inputAmount.max, feeStore);
    ...
    const sourceSwapOutputRequired = applyBuffer(bridgeOutput.add(estimatedBridgeFees), BUFFER_EXACT_OUT.SOURCE_SWAP);
    logger$4.debug('exact-out:3', {
        dstChainCOTAddress,
    ...
    const { quotes: sourceSwapQuotes, usedCOTs } = await caCommon.autoSelectSourcesV2(userAddressInBytes, sortedBalances.map((balance) => ({
        ...
        value: balance.value,
    })), sourceSwapOutputRequired, params.aggregators);
    const sourceSwaps = sourceSwapQuotes.map((v) => {
        const balance = balances.find((b) => equalFold(b.tokenAddress, convertTo32BytesHex(v.req.inputToken)));
        if (!balance) {
    ...
    const isBridgeRequired = !(sourceSwapQuotes.every((q) => Number(q.originalHolding.chainID.chainID) === input.toChainId) &&
    ...
    const dstChainExistingCOT = usedCOTs.find((c) => Number(c.originalHolding.chainID.chainID) === input.toChainId);
    ...
    let dstTotalCOTAmount = new Decimal__default.default(dstEOAToEphTx ? divDecimals(dstEOAToEphTx.amount, dstChainCOT.decimals) : 0);
    for (const cot of usedCOTs) {
        assetsUsed.push({
            ...
            symbol: caCommon.CurrencyID[cot.cur.currencyID],
        });
    ...
    for (const swap of sourceSwaps) {
        ...
        if (!token) {
            throw Errors.tokenNotFound(convertToEVMAddress(swap.req.outputToken), Number(swap.req.chain.chainID));
        }
        if (bAsset) {
        ...
        logger$4.debug('determineSwapRoute:sourceSwap', {
    ...
    logger$4.debug('exactOut:BeforeBridgeSet', {
    ...
    if (bridgeInput) {
        ...
        logger$4.debug('ExactOut: createIntent', { bridgeAssets, bridgeInput, createIntentResponse });
    }
    return {
        source: {
            swaps: sourceSwaps,
...
const normalizeToComparisonAddr = (tokenHex) => convertTo32BytesHex(equalFold(tokenHex, ZERO_ADDRESS) ? EADDRESS : tokenHex);
const _exactInRoute = async (input, params) => {
    logger$4.debug('exactInRoute', {
        input,
        params,
    });
    ...
    if (input.from.length > 0) {
        ...
        for (const f of input.from) {
            ...
            const srcBalance = balances.find((b) => {
                logger$4.debug('ExactIn: from comparison', {
            ...
            if (!srcBalance) {
                logger$4.error('ExactIN: no src balance found', {
            ...
            if (requiredBalance.gt(srcBalance.amount)) {
                throw Errors.insufficientBalance(`available: ${srcBalance.amount} ${srcBalance.symbol}, required: ${requiredBalance.toFixed()} ${srcBalance.symbol}`);
            }
            srcBalances.push({
                ...srcBalance,
    ...
    if (isSrcSwapRequired) {
        ...
        sourceSwaps = response.quotes.map((oq) => {
            const balance = balances.find((b) => equalFold(b.tokenAddress, convertTo32BytesHex(oq.req.inputToken)));
            if (!balance) {
                logger$4.error('ExactIN: failed to map quote originalHolding to balance', {
                    quoteReq: oq.req,
                });
                throw Errors.internal('mapping error: balance for quote input not found');
            }
    ...
    logger$4.debug('beforeDDS: ExactIN', {
        dstSwapInputAmountInDecimal: dstSwapInputAmountInDecimal.toFixed(),
    });
    let dstEOAToEphTx = null;
    const fetchDestinationSwapDetails = async () => {
        ...
        logger$4.debug('getDDS: ExactIN: Before', {
            dstSwapInputAmountInDecimal: dstSwapInputAmountInDecimal.toFixed(),
        });
        ...
        if (!equalFold(input.toTokenAddress, dstChainCOTAddress)) {
            destinationSwap = await caCommon.destinationSwapWithExactIn(userAddressInBytes, dstOmniversalChainID, mulDecimals(dstSwapInputAmountInDecimal, dstChainCOT.decimals), convertTo32Bytes(input.toTokenAddress), params.aggregators, dstChainCOT.currencyID);
        ...
        logger$4.debug('ExactIN: getDDS: SingleSrcSwap: After', {
            destinationSwap,
            dstSwapInputAmountInDecimal: dstSwapInputAmountInDecimal.toFixed(),
        });
        return {
    ...
    const destinationSwap = await fetchDestinationSwapDetails();
    logger$4.debug('getSwapRoute: ExactIN: After', {
        destinationSwap,
        dstSwapInputAmountInDecimal: dstSwapInputAmountInDecimal.toFixed(),
    });
    return {
...
const estimateCollectionFee = (assets, outputAmount, feeStore) => {
    const expectedAmount = applyBuffer(outputAmount, 30);
    logger$3.debug('sumCollectionFee', {
        assets,
        feeStore,
        outputAmount: outputAmount.toFixed(),
        expectedAmount: expectedAmount.toFixed(),
    });
    let accounted = new Decimal__default.default(0);
    let fee = new Decimal__default.default(0);
    for (const asset of assets) {
        ...
        logger$3.debug('estimateCollectionFee', {
...
const createIntent = ({ assets, feeStore, output, address, }) => {
    ...
    intent.destination.amount = new Decimal__default.default(borrow);
    ...
    logger$3.debug('createBridgeRFF:1', {
        assets: assets.map((i) => ({
            ...i,
    ...
    const sortedAssets = [...assets].sort((a, b) => {
        if (a.chainID === MAINNET_CHAIN_IDS.ETHEREUM)
            return 1; // a goes to end
        if (b.chainID === MAINNET_CHAIN_IDS.ETHEREUM)
            return -1; // b goes to end
        return Decimal__default.default.sub(Decimal__default.default.add(a.eoaBalance, a.ephemeralBalance), Decimal__default.default.add(b.eoaBalance, b.ephemeralBalance)).toNumber(); //sort others by balance
    });
    for (const asset of sortedAssets) {
        ...
        if (Decimal__default.default.add(asset.eoaBalance, asset.ephemeralBalance).lte(unaccountedBalance2)) {
            logger$3.debug('createBridgeRFF:2.1', {
            ...
            if (borrowFromThisChain.gt(asset.ephemeralBalance.toString())) {
                logger$3.debug('createBridgeRFF:2.2', {
    ...
    if (accountedBalance < borrow) {
        throw Errors.insufficientBalance(`available: ${accountedBalance.toFixed()}, required: ${borrow.toFixed()}`);
    }
    logger$3.debug('createIntentEnd', {
...
const createBridgeRFF = async ({ config, input, output, }) => {
    logger$3.debug('createBridgeRFF', { input, output });
    ...
    logger$3.debug('createIntent', { intent });
    ...
    const createRFF = async () => {
        ...
        for (const s of omniversalRFF.protobufRFF.sources) {
            doubleCheckTxMap[viem.bytesToNumber(s.chainID)] = createDoubleCheckTx(s.chainID, config.cosmos, intentID);
        }
        return {
            createDoubleCheckTx: async () => {
                try {
                    for (const k in doubleCheckTxMap) {
                        logger$3.debug('Starting double check tx', { chain: k });
                        await doubleCheckTxMap[k]();
                    }
                }
                catch (error) {
                    logger$3.error('Error during double check tx', error);
                }
            },
            intentID,
        };
    ...
    for (const [index, source] of sources.entries()) {
        ...
        logger$3.debug('allowances', { allowance, chainID: Number(source.chainID) });
        if (allowance == null) {
        ...
        if (allowance < source.valueRaw) {
            const allowanceTx = {
                data: packERC20Approve(config.chainList.getVaultContractAddress(Number(source.chainID)), source.valueRaw),
    ...
    const waitForFill = () => {
        ...
        logger$3.debug(`Waiting for fill: ${intentID}`);
        const r = {
            filled: false,
            intentID,
            promise: evmWaitForFill(config.chainList.getVaultContractAddress(chain.id), pc, s.requestHash, intentID, config.cosmosQueryClient),
        };
        r.promise.then(() => {
            r.filled = true;
...
const createDoubleCheckTx = (chainID, cosmos, intentID) => {
    const msg = caCommon.MsgDoubleCheckTx.create({
        creator: cosmos.address,
        packet: {
            $case: 'depositPacket',
    ...
    async createRFFDeposits() {
        ...
        if (Object.keys(this.depositCalls).length > 0) {
            ...
            if (sbcTx.length) {
                ...
                waitingPromises.push(...ops.map(([chainID, hash]) => {
                    const chain = this.options.chainList.getChainByID(Number(chainID));
                    if (!chain) {
                        throw Errors.chainNotFound(chainID);
                    }
                    this.options.emitter.emit(SWAP_STEPS.BRIDGE_DEPOSIT({
                        chain,
                        hash,
                        explorerURL: chain.blockExplorers.default.url,
                    }));
                    return wrap(Number(chainID), waitForTxReceipt(hash, this.options.publicClientList.get(chainID), 1));
                }));
            }
        }
...
class DestinationSwapHandler {
    ...
    options;
    eoaToEphCalls = [];
    constructor(data, dstTokenInfo, dst, options) {
        this.data = data;
        ...
        logger$2.debug('dstSwapHandler:constructor', {
            isNativeAddress: isNativeAddress$1(dst.token),
        });
        if (isNativeAddress$1(dst.token)) {
    ...
    // inputAmount = this.dstSwap.quote?.inputAmount,
        ...
        try {
            ...
            logger$2.warn('Destination swap failed, attempting single requote & retry.', {
            ...
            try {
                ...
                logger$2.error('Destination swap failed even after retry.', {
    ...
    async executeSwap(metadata) {
        ...
        logger$2.debug('before dst metadata', { metadata });
     ...
     */
    async requoteIfRequired(force = false) {
        ...
        let requote = force;
        if (!force) {
            if (swap.tokenSwap.aggregator instanceof caCommon.BebopAggregator) {
                const quote = swap.tokenSwap.quote;
        ...
        logger$2.debug('Requoting destination swap...');
        ...
        logger$2.debug('destinationSwap Requote', {
            isExactIn,
        ...
        if (!isExactIn &&
            ...
            throw Errors.ratesChangedBeyondTolerance(rate, `${swap.inputAmount.min} - ${swap.inputAmount.max}`);
        }
        this.data = {
            type: this.data.type,
            swap: newSwap,
        ...
        logger$2.debug('Destination swap requoted successfully.', {
...
class SourceSwapsHandler {
    ...
    *iterate(input) {
    ...
    async process(metadata, input = this.swapsData, retry = true) {
        ...
        for (const [chainID, swapQuotes] of this.iterate(input)) {
            ...
            for (const quote of quotes) {
                ...
                if (isNativeAddress$1(convertToEVMAddress(quote.input.token))) {
                    sbcCalls.value += quote.input.amount;
                }
                else {
                    this.options.emitter.emit(SWAP_STEPS.CREATE_PERMIT_FOR_SOURCE_SWAP(false, quote.input.symbol, chain));
                    ...
                    this.options.emitter.emit(SWAP_STEPS.CREATE_PERMIT_FOR_SOURCE_SWAP(true, quote.input.symbol, chain));
            ...
            if (sbcCalls.value > 0n) {
                ...
                this.options.emitter.emit(SWAP_STEPS.SOURCE_SWAP_HASH([BigInt(chain.id), hash], this.options.chainList));
                ...
                waitingPromises.push((async () => {
                    ...
                    metadataTx.tx_hash = convertTo32Bytes(hash);
                    this.options.emitter.emit(SWAP_STEPS.SOURCE_SWAP_HASH([chainID, hash], this.options.chainList));
                    return wrap(Number(chainID), waitForTxReceipt(hash, this.options.publicClientList.get(chainID), 1));
                })());
            }
            assets.push({
                amount: divDecimals(amount, getTokenDecimals(Number(chainID), quotes[0].output.token).decimals),
                chainID: Number(chainID),
...
class Swap {
    ...
    getParsedQuote() {
        const data = parseQuote({ ...this.input, inputToken: this.input.req.inputToken }, !bytesEqual(EADDRESS_32_BYTES, this.input.req.inputToken));
...
const swap = async (input, options, COT = caCommon.CurrencyID.USDC) => {
    ...
    // Swap Intent hook handling
    {
        const destinationTokenDetails = {
            amount: divDecimals(input.mode === exports.SwapMode.EXACT_OUT
                ? input.data.toAmount
                : destination.swap.tokenSwap.outputAmount, dstTokenInfo.decimals).toFixed(),
            chainID: input.data.toChainId,
            contractAddress: input.data.toTokenAddress,
            decimals: dstTokenInfo.decimals,
            symbol: dstTokenInfo.symbol,
            gasAmount: divDecimals(destination.swap.gasSwap?.quote?.outputAmountMinimum ?? '0', dstChain.nativeCurrency.decimals).toFixed(),
        };
        let accepted = false;
        const refresh = async (fromSources) => {
            if (accepted) {
            ...
            if (updatedInput.mode === exports.SwapMode.EXACT_OUT && fromSources && fromSources.length > 0) {
            ...
            const swapIntent = createSwapIntent(extras.assetsUsed, destinationTokenDetails, options.chainList);
            logger$1.debug('onIntentHook:refresh', { swapIntent });
            return swapIntent;
        };
        // wait for intent acceptance hook
        await new Promise((resolve, reject) => {
            ...
            const swapIntent = createSwapIntent(extras.assetsUsed, destinationTokenDetails, options.chainList);
            logger$1.debug('onIntentHook', { swapIntent });
    ...
    const dstSwapHandler = new DestinationSwapHandler(destination, dstTokenInfo, {
        chainID: input.data.toChainId,
        token: input.data.toTokenAddress,
        amount: input.mode === exports.SwapMode.EXACT_OUT
            ? input.data.toAmount
...
(function (INIT_STATUS) {
    INIT_STATUS[INIT_STATUS["CREATED"] = 1] = "CREATED";
    INIT_STATUS[INIT_STATUS["RUNNING"] = 2] = "RUNNING";
    INIT_STATUS[INIT_STATUS["DONE"] = 3] = "DONE";
})(INIT_STATUS || (INIT_STATUS = {}));
const SIWE_STATEMENT = 'Sign in to enable Nexus';
class CA {
    static getSupportedChains = getSupportedChains$1;
    #cosmos;
    #ephemeralWallet;
    chainList;
    _siweChain;
    ...
    _initStatus = INIT_STATUS.CREATED;
    _networkConfig;
    _refundInterval;
    _initPromise = null;
    simulationClient;
    _analytics; // Analytics manager set by subclass
    constructor(config = {
        debug: false,
        network: 'testnet',
        siweChain: 1,
        ...
        const defaultChain = this._networkConfig.NETWORK_HINT === caCommon.Environment.FOLLY
            ? SUPPORTED_CHAINS.SEPOLIA
            : SUPPORTED_CHAINS.ETHEREUM;
    ...
    _createBridgeHandler = (input, options) => {
        if (!this._evm) {
        ...
        return this.withReinit(async () => {
            return new BridgeHandler$1(params, {
    ...
    _calculateMaxForBridge = async (params) => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
    ...
    _getBalancesForSwap = async (onlyNativesAndStables = false) => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
        return this.withReinit(async () => {
    ...
    _swapWithExactIn = async (input, options) => {
        return this.withReinit(async () => {
            return swap({
                mode: exports.SwapMode.EXACT_IN,
                data: input,
            }, await this._getSwapOptions(options));
        });
    };
    _swapWithExactOut = async (input, options) => {
        return this.withReinit(async () => {
            ...
            return swap({
                mode: exports.SwapMode.EXACT_OUT,
                data: input,
            }, await this._getSwapOptions(options));
        });
    };
    _swapAndExecute = async (input, options) => {
        return this.withReinit(async () => {
            return new SwapAndExecuteQuery(this.chainList, this._evm.client, this._getBalancesForSwap, this._swapWithExactOut).swapAndExecute(input, options);
        });
    };
    _getSwapOptions = async (options) => {
        return {
    ...
    async _setEVMProvider(provider) {
        ...
        try {
            const client = viem.createWalletClient({
                transport: viem.custom({ ...provider, request: provider.request.bind(provider) }),
            });
    ...
    _setTronAdapter = async (adapter) => {
        ...
        this._tron = {
            adapter,
            address: tronHexToEvmAddress(tronweb.utils.address.toHex(adapter.address)),
        };
    };
    ...
    _createCosmosWallet = async () => {
        ...
        const wallet = await caCommon.createCosmosWallet(`0x${pvtKey.padStart(64, '0')}`);
        this.#ephemeralWallet = accounts.privateKeyToAccount(`0x${pvtKey.padStart(64, '0')}`);
        const address = (await wallet.getAccounts())[0].address;
        await cosmosFeeGrant(address, this._queryClients.cosmosQueryClient, this._queryClients.vscClient);
        const client = await caCommon.createCosmosClient(wallet, this._networkConfig.COSMOS_RPC_URL, {
            broadcastPollIntervalMs: 250,
        });
        return { wallet, address, client };
    };
    _getCosmosWallet = async () => {
        if (!this.#cosmos) {
            this.#cosmos = await this._createCosmosWallet();
        }
        return this.#cosmos;
    };
    _getEVMAddress = async () => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
        return (await this._evm.client.requestAddresses())[0];
    };
    _setProviderHooks = async () => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
    ...
    _getSwapSupportedChains = () => {
        return getSwapSupportedChains(this.chainList);
    };
    _simulateBridgeAndExecute = (params) => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
        return this.withReinit(() => {
            return new BridgeAndExecuteQuery(this.chainList, this._evm.client, this._createBridgeHandler, this._getBalancesForBridge, this.simulationClient).simulateBridgeAndExecute(params);
        });
    };
    _bridgeAndExecute = (params, options) => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
        return this.withReinit(() => {
            return new BridgeAndExecuteQuery(this.chainList, this._evm.client, this._createBridgeHandler, this._getBalancesForBridge, this.simulationClient).bridgeAndExecute(params, options);
        });
    };
    _execute = async (params, options) => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
        return this.withReinit(() => {
            return new BridgeAndExecuteQuery(this.chainList, this._evm.client, this._createBridgeHandler, this._getBalancesForBridge, this.simulationClient).execute(params, options);
        });
    };
    _simulateExecute = (params) => {
        if (!this._evm) {
            throw Errors.sdkNotInitialized();
        }
        return this.withReinit(() => {
            return new BridgeAndExecuteQuery(this.chainList, this._evm.client, this._createBridgeHandler, this._getBalancesForBridge, this.simulationClient).simulateExecute(params, this._evm.address);
        });
    };
    _triggerAccountChange = async () => {
        await this.reinitOnAccountChange();
    };
    universeCheck = (dstChain) => {
        if (dstChain.universe === caCommon.Universe.TRON && !this._tron) {
            throw Errors.walletNotConnected('Tron');
        }
    };
    reinitOnAccountChange = async () => {
        if (!this._evm) {
            return;
        }
    ...
    _convertTokenReadableAmountToBigInt = (amount, tokenSymbol, chainId) => {
...
const formatTokenBalance = formatTokenBalance$1;
const formatTokenBalanceParts = formatTokenBalanceParts$1;
const truncateAddress = truncateAddress$1;
const parseUnits = viem.parseUnits;
const formatUnits = viem.formatUnits;
const isValidAddress = viem.isAddress;
const getCoinbaseRates = async () => getCoinbasePrices();
const getSupportedChains = (env) => getSupportedChains$1(env);
...
class NexusSDK extends CA {
    ...
    constructor(config) {
        super(config);
        logger$e.debug('Nexus SDK initialized with config:', config);
        this.utils = new NexusUtils(this.chainList);
        // Initialize analytics (backwards compatible - enabled by default)
        this.analytics = new AnalyticsManager(getNetwork(config?.network || 'mainnet'), config?.analytics);
    ...
    async bridge(params, options) {
        ...
        try {
            ...
            this.analytics.endOperation(opId, { success: false, error: error });
            throw error;
        }
    }
    /**
     ...
     * @returns
     */
    async calculateMaxForBridge(params) {
     ...
     */
    async bridgeAndTransfer(params, options) {
     ...
     * @returns simulation result with gas estimates
     */
    async simulateBridgeAndTransfer(params) {
     ...
     */
    async getBalancesForSwap(onlyNativesAndStables = false) {
    ...
    swapAndExecute = this._swapAndExecute;
    /**
     ...
     */
    setEVMProvider = this._setEVMProvider;
    /**
     ...
     */
    triggerAccountChange = this._triggerAccountChange;
    /**
     ...
     */
    convertTokenReadableAmountToBigInt = this._convertTokenReadableAmountToBigInt;
    /**
     * Helper function to initiate refund if not already triggered by system
     */
    refundIntent = this._refundIntent;
}

exports.AnalyticsManager = AnalyticsManager;
exports.BRIDGE_STEPS = BRIDGE_STEPS;
...
exports.LOG_LEVEL_NAME = LOG_LEVEL_NAME;
exports.MAINNET_CHAINS = MAINNET_CHAINS$1;
exports.MAINNET_CHAIN_IDS = MAINNET_CHAIN_IDS;
exports.NEXUS_EVENTS = NEXUS_EVENTS;
exports.NexusAnalyticsEvents = NexusAnalyticsEvents;
exports.NexusError = NexusError;
exports.NexusSDK = NexusSDK;
exports.SUPPORTED_CHAINS = SUPPORTED_CHAINS;
exports.SWAP_STEPS = SWAP_STEPS;
exports.TESTNET_CHAINS = TESTNET_CHAINS$1;
exports.TESTNET_CHAIN_IDS = TESTNET_CHAIN_IDS;
...
exports.formatTokenBalanceParts = formatTokenBalanceParts;
exports.formatUnits = formatUnits;
exports.getCoinbaseRates = getCoinbaseRates;
exports.getLogger = getLogger;
exports.getSupportedChains = getSupportedChains;
exports.isSupportedToken = isSupportedToken;
exports.isValidAddress = isValidAddress;
exports.logger = logger$e;
